(ns kontor.integration.cross-border-scenario-test
  "End-to-end integration test promoting the Phase D REPL walk
 into a permanent regression. Exercises:

   - **Two-DB topology**: one DE kontor DB for the Hans-Tech UG, one
     CA kontor DB for personal-CA (sole-prop + investments).
  ...
  (let [conn (de-preset/create-de-db)]
    (d/transact conn
                [{:kontor.entity/name "Hans-Tech UG (haftungsbeschränkt)"
                 ...
                 {:kontor.partner/name "Sample Owner" :kontor.partner/external-id "OWNER"
...
(defn- hans-db
  "personal-CA CA DB: preset + entity + counterparty partner +
   the foreign-tax accounts the treaty helper writes to."
  []
  (let [conn (ca-preset/create-ca-db)]
    (d/transact conn
                [{:kontor.entity/name "Sample Owner (Individual)" :kontor.entity/code "OWNER-PERSONAL"
    ...
    (e {:journal [:kontor.journal/code "GJ"] :effective-date #inst "2026-12-31"
        ...
        :postings [{:account [:kontor.account/path "Eigenkapital:Gewinnvortrag"]            :amount 15000M}
                   {:account [:kontor.account/path "Verbindlichkeiten:Dividenden-Zahlbar"] :amount -9000M
                    :partner [:kontor.partner/external-id "OWNER"]}
...
(deftest ug-dividend-per-shareholder-allocation-via-i15
  ...
  (let [conn (ug-db)
        ...
        pairs (set (d/q '[:find ?path ?amt ?pc
                        ...
                        (d/db conn)))]
    (is (contains? pairs ["Verbindlichkeiten:Dividenden-Zahlbar" -9000M "OWNER"])
        "the owner's Cr leg carries :kontor.posting/partner CW")
...
(deftest hans-side-cross-db-dividend-via-treaty-helper
  ;; The companion-to-the-corp side. the owner's CA DB receives the
  ...
  (let [conn (hans-db)
        _    (book/entry! conn   ; First the sole-prop revenue (CAD 60k + 5 % GST)
                          {:journal [:kontor.journal/code "CR"] :effective-date #inst "2026-09-30"
                           :commodity :CAD :entity [:kontor.entity/code "OWNER-PERSONAL"]
        ...
        _    (treaty/receive-dividend-from-de! conn
                                               {:gross-amount    9000M
                                                ...
                                                :entity          [:kontor.entity/code "OWNER-PERSONAL"]})
