(ns kontor.banking.payment-application-test
...
(defn- bootstrap [f]
  (binding [*conn* (core/create-test-db)]
    ;; The :kontor.invoice/status transitions seeded by the companion schema
    ;; install. Kernel itself doesn't ship :kontor.invoice/status state
...
(deftest full-payment-flips-sent-to-paid
  (testing "Single application equal to invoice gross transitions
    ...
    (let [inv (make-invoice! "INV-A" 100M)
      ...
      (let [db (d/db *conn*)]
        (testing ":kontor.invoice/status → :paid"
  ...
  (testing "Reversal of the only application moves :paid → :sent"
    (let [inv (make-invoice! "INV-C" 500M)
      ...
      (let [db (d/db *conn*)
        ...
        (let [db2 (d/db *conn*)]
          (testing ":kontor.invoice/status → :sent"
            (is (= :sent (sm/current-status db2 inv :kontor.invoice/status))))
    ...
    (let [inv (make-invoice! "INV-RP1" 600M)
      ...
      (let [db (d/db *conn*)]
        (testing ":partially-paid → :sent after last reversal"
          (is (= :sent (sm/current-status db inv :kontor.invoice/status))))
        (testing "net applied = 0"
      ...
      (let [allocs (papp/allocate-fifo! *conn*
        ...
        (let [db (d/db *conn*)]
          (testing "INV-OLD fully paid (got 300)"
            (is (= :paid (sm/current-status db inv-old :kontor.invoice/status)))
            (is (= 0 (.compareTo 0M (papp/open-amount-of-invoice db inv-old)))))
          (testing "INV-NEW partially paid (got 300 of 500; open = 200)"
            (is (= :partially-paid (sm/current-status db inv-new :kontor.invoice/status)))
