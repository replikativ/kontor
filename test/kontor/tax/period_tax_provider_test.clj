(ns kontor.tax.period-tax-provider-test
  "Iteration 1 of the period-tax build (ADR-099, research note 102) —
   the substrate: the `kontor.tax.tax-schedule` algebra, the
...
;; ============================================================================
;; ADR-099 addendum — base transform + component combinators
;; ============================================================================
...
(defn- const-fx-provider
  ...
  [rate]
  (reify kontor.fx.fx_rate_provider.FxRateProvider
...
;; ============================================================================
;; assert-monocommodity-facts! — the adoption-pattern wrapper for P1.b
;;
;; The check itself (monocommodity-facts? above) is the predicate; this is
;; the throw-or-return-facts wrapper providers actually use. Note 168 S1
;; P1.b adoption sequence step (b) — DE/FR/JP/CA CIT providers wrap their
;; tax-return-facts builder calls with this so a future change that
;; introduces foreign commodities (without translating) is loud.
;; ============================================================================

(deftest assert-monocommodity-facts!-returns-facts-on-pass
  (testing "When facts are monocommodity, the wrapper is a no-op that
            returns the same facts (so providers can wrap in-line)."
    (let [facts (ptp/tax-return-facts
                 {:entity 1 :period {:from #inst "2026-01-01" :to #inst "2026-12-31"}
                  :jurisdiction {:country :de}
                  :functional-commodity "EUR"
                  :components [{:kind :corporate-income-tax
                                :base (money/->Money 100000M "EUR")
                                :liability (money/->Money 15000M "EUR")}]})]
      (is (identical? facts (ptp/assert-monocommodity-facts! facts))))))

(deftest assert-monocommodity-facts!-throws-on-violation
  (testing "When facts span more than one commodity, the wrapper throws
            a structured :tax/multi-commodity-facts ex-info with the
            offending components surfaced for debugging."
    (let [bad (ptp/tax-return-facts
               {:entity 1 :period {:from #inst "2026-01-01" :to #inst "2026-12-31"}
                :jurisdiction {:country :de}
                :functional-commodity "EUR"
                :components [{:kind :corporate-income-tax
                              :base (money/->Money 100000M "CHF")
                              :liability (money/->Money 15000M "EUR")}]})
          ex  (try (ptp/assert-monocommodity-facts! bad)
                   nil
                   (catch clojure.lang.ExceptionInfo e e))]
      (is (some? ex))
      (is (= :tax/multi-commodity-facts (:type (ex-data ex))))
      (is (= "EUR" (:functional-commodity (ex-data ex))))
      (is (= 1 (count (:components (ex-data ex))))))))
