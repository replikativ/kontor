(ns kontor.bitemporal
  "Thin write-side bitemporal shim over datahike's upstream
   ...
   `strip-tx-meta` because `kontor.workflow.process` uses it during
...
(defn strip-tx-meta
  "Remove any existing `{:db/id \"datomic.tx\" ...}` map(s) from
   ...
   `kontor.workflow.process/run-steps` to strip per-fragment tx-meta as it
...
;; with `kontor.workflow.process` step lists; the wrapper is the convenience
;; for one-shot use.
;;
...
(defn close-validity!
  "Side-effecting wrapper for [[close-validity-tx-data]]. Routes
   through `kontor.gate/transact-with-validation` so kernel
  ...
  (gate/transact-with-validation conn (close-validity-tx-data prior-tx-eid vt)))
