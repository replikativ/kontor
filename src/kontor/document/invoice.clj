(ns kontor.document.invoice
  "Kernel-side **invoice-as-document** — lifecycle (create / send /
   ...
   Totals: caller may supply :kontor.invoice/total-net etc. directly, OR
...
(defn- line-net
  ^java.math.BigDecimal [{:kontor.invoice-line/keys [quantity unit-price]}]
  (.setScale (.multiply (bigdec quantity) (bigdec unit-price))
             2 java.math.RoundingMode/HALF_EVEN))

(defn- line-vat
  ^java.math.BigDecimal [{:kontor.invoice-line/keys [vat-rate] :as line}]
...
(defn materialize-totals
  "Compute :kontor.invoice/total-net + :total-vat + :total-gross from the
  ...
  (let [net (reduce #(.add ^java.math.BigDecimal %1 (line-net %2)) 0M lines)
    ...
    {:kontor.invoice/total-net   net
     :kontor.invoice/total-vat   vat
     :kontor.invoice/total-gross (.add net vat)}))
...
(defn create-tx-data
  ...
  [db invoice-map]
  (when-not (:kontor.invoice/external-id invoice-map)
    (throw (ex-info ":kontor.invoice/external-id is required" {:invoice invoice-map})))
  (when-not (:kontor.invoice/issue-date invoice-map)
    (throw (ex-info ":kontor.invoice/issue-date is required" {:invoice invoice-map})))
  (when-not (seq (:kontor.invoice/lines invoice-map))
    (throw (ex-info ":kontor.invoice/lines must be non-empty" {:invoice invoice-map})))
  (let [lines (:kontor.invoice/lines invoice-map)
        tempid (str "invoice-" (:kontor.invoice/external-id invoice-map))
        ;; Materialize totals from lines unless caller supplied them.
        totals (cond-> {}
                 (not (:kontor.invoice/total-net   invoice-map)) (merge (materialize-totals lines)))
        ...
        term-frag (when-let [term-eid (:kontor.invoice/payment-term invoice-map)]
                    (let [term (d/pull db [:db/id :kontor.payment-term/net-days
                          ...
                          rename {:kontor.transaction/payment-term :kontor.invoice/payment-term
                                  :kontor.transaction/due-date     :kontor.invoice/due-date
                                  :kontor.transaction/discount-deadline :kontor.invoice/discount-deadline}]
                      (into {} (map (fn [[k v]] [(rename k k) v])) frag)))
        ;; Build line-item entity maps with sequential tempids and a
        ;; :kontor.invoice-line/invoice backref to the parent.
        line-tx (mapv (fn [i line]
                        (-> line
                            (dissoc :kontor.invoice/lines)
                            (assoc :db/id (str "line-" (:kontor.invoice/external-id invoice-map) "-" i)
                                   :kontor.invoice-line/invoice tempid
                                   :kontor.invoice-line/sequence (or (:kontor.invoice-line/sequence line) (inc i)))))
                      (range)
                      lines)
        ...
        invoice-tx (-> invoice-map
                       ...
                       (assoc :db/id tempid
                              ...
                              :kontor.invoice/currency (or (:kontor.invoice/currency invoice-map) "EUR"))
                       (merge totals)
                       (merge term-frag)
                       (assoc :kontor.invoice/lines (mapv :db/id line-tx)))]
    (into [invoice-tx] line-tx)))

(defn create!
  "Create a draft invoice + its line items. `invoice-map` shape:

     {:kontor.invoice/external-id   String  ; required
      :kontor.invoice/issue-date    Date    ; required
      :kontor.invoice/seller        eid     ; partner ref
      :kontor.invoice/buyer         eid     ; partner ref
      :kontor.invoice/payment-term  eid     ; payment-term ref (optional)
      :kontor.invoice/currency      String  ; default \"EUR\"
      :kontor.invoice/lines         [<line-map> …]  ; required, ≥1
      :kontor.invoice/notes         [String]
      :kontor.invoice/buyer-reference String}

   Line maps:
     {:kontor.invoice-line/sequence  long
      :kontor.invoice-line/name      String
      :kontor.invoice-line/description String
      :kontor.invoice-line/quantity  bigdec
      :kontor.invoice-line/unit-code String
      :kontor.invoice-line/unit-price bigdec
      :kontor.invoice-line/vat-rate  bigdec
      :kontor.invoice-line/vat-category String
      :kontor.invoice-line/account   eid (optional override)}
...
(defn send-tx-data
  ...
  (let [inv (d/pull db
                    [:db/id :kontor.invoice/external-id :kontor.invoice/status
                     :kontor.invoice/issue-date :kontor.invoice/currency
                     :kontor.invoice/total-net :kontor.invoice/total-vat :kontor.invoice/total-gross
                     :kontor.invoice/buyer-reference
                     {:kontor.invoice/seller [:db/id :kontor.partner/external-id :kontor.partner/name
                                              :kontor.partner/tax-id]}
                     {:kontor.invoice/buyer  [:db/id :kontor.partner/external-id :kontor.partner/name
                                              :kontor.partner/tax-id]}
                     {:kontor.invoice/lines [:kontor.invoice-line/sequence
                                             :kontor.invoice-line/name
                                             :kontor.invoice-line/description
                                             :kontor.invoice-line/quantity
                                             :kontor.invoice-line/unit-code
                                             :kontor.invoice-line/unit-price
                                             :kontor.invoice-line/vat-rate
                                             :kontor.invoice-line/vat-category
                                             {:kontor.invoice-line/account [:db/id :kontor.account/code]}]}]
                    invoice-eid)]
    (when-not (= :draft (:kontor.invoice/status inv))
    ...
    (let [now (or changed-at (Date.))
      ...
      (-> (vec tx)
          (conj {:db/id invoice-eid :kontor.invoice/transaction tx-tempid})
          (into status-tx)))))
...
(defn send!
  "Transition a :draft invoice to :sent. Calls `(posting-builder db
   ...
   resulting transaction + sets :kontor.invoice/transaction backref +
...
(defn cancel-tx-data
  ...
  [db {:keys [invoice-eid changed-by-uid reason reason-note supporting-doc
              changed-at]}]
  (let [inv (d/pull db [:kontor.invoice/status :kontor.invoice/external-id
        ...
        status-change-opts (cond-> {:entity invoice-eid
                             ...
                             reason-note    (assoc :reason-note reason-note)
                             supporting-doc (assoc :supporting-doc supporting-doc))]
    (case (:kontor.invoice/status inv)
      :draft
      (vec (sm/record-status-change-tx-data db status-change-opts))
      :sent
      (let [original-tx (:kontor.invoice/transaction inv)
            ...
            reversal-ext (str (:kontor.invoice/external-id inv) "-REV")
            reversal-tx (posting/build-transaction
                         {:transaction
                          {:kontor.transaction/external-id reversal-ext
                           ...
                           :kontor.transaction/narration (str "Reversal of " (:kontor.invoice/external-id inv))
