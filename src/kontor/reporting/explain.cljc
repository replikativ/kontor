(ns kontor.reporting.explain
  "Graph-walk helpers — \"explain this number\" — ADR-091.
   ...
   `kontor.reporting.balance` and `kontor.reporting.ledger` already walk postings against
   one account. `kontor.reporting.explain` adds the cross-namespace walk: from a
...
(defn explain-balance
  "Return `{:account :balance :postings :as-of-valid :as-of-tx}` —
   ...
   postings vector is exactly what `kontor.reporting.ledger/postings-against`
   ...
   `kontor.reporting.balance/account-balance`.
...
(defn- retention-summary-for
  "Look up retention policy + deadline + eligibility for `entity-eid`
   ...
   `entity-type` is the keyword used by `kontor.compliance.retention/policy-for`
