(ns kontor.side-effect.cross
  "Cross-DB side-effect execution — ADR-074. Generalizes
   [[kontor.side-effect]] (ADR-041) for the case where the side effect
   ...
   - Not a workflow engine. `kontor.process` is the single-DB
     orchestrator; `kontor.side-effect.cross` is the cross-DB
   ...
   - Not generic to non-datahike backends. The target conn MUST
     ...
     PUT) are still handled by the parent [[kontor.side-effect]]
...
(defn drain!
  "Execute every pending :cross-tx-post intent against the configured
   ...
   a scheduled job or a `kontor.schedule` periodic. The single-pass
