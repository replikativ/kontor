(ns kontor.workflow.event-bus
   ...
   *callback* into their code when a transaction commits.
   ...
   tx-log or to mount a `kontor.workflow.process` orchestrator and inline the
   ...
   `kontor.workflow.process/run-process` is the canonical entry point for the
   ...
   `kontor.workflow.event-bus/dispatch` is exposed publicly so a consumer who
...
(defn commit-and-emit
  "A `:commit` fn for `kontor.workflow.process/run-process` (and a drop-in
