(ns kontor.workflow.state-machine
  "Transaction-state lifecycle: enforce that `:kontor.transaction/state`
   transitions follow
   ...
   reversing transaction (`:kontor.transaction/reverses`).

   Implementation: scan tx-data for entity-map updates that set
   `:kontor.transaction/state`. For each, look up the existing state in the
   db (or treat as `nil` for brand-new entities — the implicit
   start-state). Compare against the allowed-transitions table.

   When a tx writes `:kontor.transaction/state :posted` we additionally
   require `:kontor.transaction/posted-at` to be set in the same tx — this
   triggers sealing for the postings that hang off it (those need
   :kontor.posting/posted-at too, set by the caller)."
...
(defn- proposed-transaction-state-changes
  "Walk tx-data and return a seq of {:eid :tempid? :to} for each
   entity-map writing :kontor.transaction/state. Tuple-form
   `[:db/add eid :kontor.transaction/state v]` is also handled."
  [tx-data]
  (concat
   ;; entity-map form
   (keep (fn [tx]
           (when (and (map? tx) (contains? tx :kontor.transaction/state))
             {:eid     (:db/id tx)
              :tempid? (or (nil? (:db/id tx))
                           (and (integer? (:db/id tx)) (neg? (:db/id tx))))
              :to      (:kontor.transaction/state tx)
              :tx      tx}))
         tx-data)
   ;; tuple form
   (keep (fn [tx]
           (when (and (vector? tx)
                      (= :db/add (first tx))
                      (= :kontor.transaction/state (nth tx 2 nil)))
             {:eid     (second tx)
...
(defn- current-state
  "Pull the current :kontor.transaction/state for an existing eid; nil for
   tempids (treated as brand-new)."
  [db {:keys [eid tempid?]}]
  (when (and (not tempid?) (integer? eid))
    (:kontor.transaction/state (d/pull db [:kontor.transaction/state] eid))))
             ...
             (cond
               ...
               {:tx tx :from from :to to
                ...
                :message (str "transition " (pr-str from) " → "
                 ...
                 ;; sibling [:db/add eid :kontor.transaction/posted-at ...].
               (and (= to :posted)
                    (not (and (map? tx) (:kontor.transaction/posted-at tx)))
                    (not (some #(and (vector? %)
                                     (= :db/add (first %))
                                     (= eid (second %))
                                     (= :kontor.transaction/posted-at (nth % 2 nil)))
                               tx-data)))
               {:tx tx :from from :to to
                :reason :state-machine/missing-posted-at
                :message ":kontor.transaction/state :posted requires
                           :kontor.transaction/posted-at to be set in the
...
(defn assert-transition!
  "Throws ex-info :type :state-machine/violation on any illegal
   :kontor.transaction/state transition or missing posted-at requirement."
  [db tx-data]
