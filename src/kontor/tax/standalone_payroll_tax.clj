(ns kontor.tax.standalone-payroll-tax
  "A generic `PeriodTaxProvider` for standalone employer payroll taxes
   ...
   The base-selector IS `kontor.reporting.report` — the σ_E of ADR-096: the wage
...
(defn standalone-payroll-tax-provider
  "Construct a `StandalonePayrollTaxProvider`. Config keys:
     ...
     :schedule    — a `kontor.tax.tax-schedule` (`flat` / `capped` / …)
