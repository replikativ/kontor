(ns kontor.tax.cgt
  "Composition helpers for CGT → PIT/CIT integration — ADR-103
   ...
   silent-tax-miss-prone — the CA review caught this as
   a substrate gap.
   ...
   consume natively (via `kontor.tax.tax-schedule/apply-base-transform`).

   ## Jurisdiction-specific shapes — NOT handled here

   AT uses a tagged-map shape for §30 Abs 7 loss carry:
   ...
   ## Why not put this in `kontor.provider.disposal-provider`?

   `DisposalProvider` is the data-source protocol (CGT providers depend
   on it for events). `kontor.tax.cgt` is the CONSUMER-side composition
