(ns kontor.tax.period-tax-provider
   ...
   *period/entity-incident* taxes — personal and corporate income tax,
   ...
   - `TaxReturnFacts` — the inter-protocol data contract (mirrors
     ...
     enum.
   - the base-selector — `kontor.reporting.report/marginalize` (ADR-096) IS the
     period tax's base; a flow base is a windowed σ_E, a wealth-tax
     stock base a cumulative roll-up. note 102 §3b.
   - the schedule — `kontor.tax.tax-schedule/apply-schedule`.
   - `kontor.tax.tax-return-posting-builder/TaxReturnPostingBuilder` — the
...
;; ============================================================================
;; The closed period-tax :kind enum
;; ============================================================================
...
(def period-tax-kinds
  ...
  #{:personal-income-tax        ; CA T1, DE Einkommensteuer, US 1040
    ...
    :branch-or-presumptive-tax}) ; presumptive / imputed-income regimes —
                                ;   base not in the books

;; ============================================================================
;; TaxReturnFacts — the inter-protocol data contract
;; ============================================================================
...
;;    :base-transform <data>  optional kontor.tax.tax-schedule transform from the
;;                            marginalized aggregate to :base — corporate
;;                            add-backs / BR Lucro Presumido (ADR-099
;;                            addendum, note 103 GAP 1)
;;    :schedule     <data>    the kontor.tax.tax-schedule that produced the gross
...
;;    :regime       <kw|nil>  which elective regime applied
;;    :composed-of  [<kw>]    other component :kinds this one is derived from —
;;                            for surtaxes / minimum taxes
    ...
    "Given an entity × period `context`, return a `TaxReturnFacts`
     ...
     Context keys:
       ...
       :tax-unit     — optional household/filing-unit descriptor, for
                       ...
                       note 102 §9-D)
       :fx-provider  — optional kontor.fx.fx-rate-provider/FxRateProvider.
...
. The capital-loss carry-in uses a
        ...
        (when (nil? fx-provider)
          (throw (ex-info
                  (str "translate-to-functional: base in " (pr-str from)
                       ...
                       "Add an FxRateProvider (see kontor.fx.fx-rate-provider).")
...
(defn translate-amounts-to-functional
  "Convenience over [[translate-to-functional]] for a `{commodity →
   BigDecimal}` summary (the shape `kontor.reporting.report/marginalize` returns
...
(defn assert-monocommodity-facts!
  "Post-construction guard for [[tax-return-facts]] output. Returns
   `facts` unchanged when [[monocommodity-facts?]] passes; throws a
   structured `:tax/multi-commodity-facts` ex-info otherwise.

   Adoption pattern for note 168 S1 P1.b — each per-l10n provider
   wraps its facts builder so a future change introducing foreign
   commodities into the output (without an FX translation step) is
   loud rather than silently emitting wrong-currency liabilities.

   Usage:
     (ptp/assert-monocommodity-facts!
       (ptp/tax-return-facts {...}))"
  [facts]
  (when-not (monocommodity-facts? facts)
    (throw (ex-info "tax-return-facts is not monocommodity"
                    {:type :tax/multi-commodity-facts
                     :functional-commodity (:functional-commodity facts)
                     :components (mapv (fn [c]
                                         {:component           (:component c)
                                          :base-commodity      (some-> c :base :commodity)
                                          :liability-commodity (some-> c :liability :commodity)})
                                       (:components facts))})))
  facts)
