   ...
   The evaluator's central API:
     ...
     (apply-provisions db query ctx)
       → {:items [...] :provisions-applied [...]}
         ...
         `kontor.tax.tax-schedule/apply-base-adjustments` or
...
(defn- resolve-schedule-template
  "Resolve a schedule template embedded in a `:schedule-override`
   consequence to a concrete `kontor.tax.tax-schedule` schedule map at
...
(defn resolve-consequence
  "Turn a `:kontor.provision/consequence` EDN map into a fold-ready item the
   `kontor.tax.tax-schedule` adjustment folds will consume.
   ...
   where `:schedule` is a fully-resolved `kontor.tax.tax-schedule` data
...
(defn apply-provisions
  "Resolve applicable provisions to fold-ready items, grouped by `:op`
   ...
   The provider routes each list to the right place:
     base-items         → `kontor.tax.tax-schedule/apply-base-adjustments`
     tax-items          → `kontor.tax.tax-schedule/apply-adjustments`
...
(defn compose-aggregate-of
  "Compose two TaxReturnFacts paths — the ELECTED computation (under a
   fiscal-unit regime) and its SEPARATE counterfactual (the same
   computation done entity-by-entity, summed). The elected path
   prevails in the output; the separate path is recorded in
   `:composed-of` + `:composition` for audit, along with the economic
   delta (separate − elected, i.e. positive when electing saves
   money).

   The canonical use is fiscal-unit election validation (ADR-113): did
   electing the regime save money relative to filing separately? The
   substrate does NOT enforce that election is rational — taxpayers
   may elect for non-tax reasons (administrative simplicity,
   intra-group dividend efficiency, expected future losses). It
   merely records the comparison.

   Mirrors `compose-greater-of` but the prevailing branch is
   user-specified (the election) rather than picked by liability.
   Note 189 §5.3 — the only new statute-level primitive Gap #8 v1
   needs.

   Output preserves the elected path's structure; adds:
     :composed-of  [<kind of elected> <kind of separate>]
     :composition  {:method :aggregate-of
                    :elected :a
                    :elected-liability  <BigDecimal>
                    :separate-liability <BigDecimal>
                    :economic-delta     <BigDecimal>}

   The `:elected` key is always `:a` (the elected component is always
   the first argument) — symmetric with `compose-greater-of`'s
   `:prevailed`. ADR-113."
  [elected separate]
  (let [elected-amt  (or (some-> elected  :liability :amount) 0M)
        separate-amt (or (some-> separate :liability :amount) 0M)
        delta        (.subtract ^BigDecimal separate-amt
                                ^BigDecimal elected-amt)]
    (assoc elected
           :composed-of (mapv :kind [elected separate])
           :composition {:method             :aggregate-of
                         :elected            :a
                         :elected-liability  elected-amt
                         :separate-liability separate-amt
                         :economic-delta     delta})))
