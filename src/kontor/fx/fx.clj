(ns kontor.fx.fx
  "Currency-translation operations on `Money` values.
   ...
   [[kontor.fx.fx-rate-provider]] is the *rate*-side: it knows how to look
   up a multiplier for (from, to, date, rate-type). `kontor.fx.fx` knows
   ...
   Two operations cover today's needs:
     ...
     [[translate-amounts-by-commodity]]
       ...
       `kontor.reporting.balance/account-balance` returns), re-base into ONE
       presentation commodity using the provider. Used by reports +
       (future) `kontor.provider.consolidation/translate-currency!`.
   ...
   The substrate ships no rates; an [[kontor.fx.fx-rate-provider/FxRateProvider]]
...
(defn convert
  "Translate a `Money` into the target commodity at the rate the
   ...
   [[kontor.fx.fx-rate-provider/FxRateProvider]].
...
(defn translate-amounts-by-commodity
  "Re-base a `{commodity → BigDecimal}` summary (as returned by
   [[kontor.reporting.balance/account-balance]] or
   [[kontor.reporting.report/compute-report]]'s per-commodity rollup) into ONE
