(ns kontor.provider.costing-provider
  "CostingProvider protocol + kernel-shipped implementations — ADR-029.

   Direct sibling of `kontor.tax.tax-rate-provider`. The kernel ships four
  ...
  (:require [kontor.provider.valuation :as valuation]))

(defn- view-opts
  "Build the opts map passed to kontor.provider.valuation view helpers from a
   request. Currently extracts :as-of-valid + :include-states so the
   layer queries honor the same bitemporal contract as
   kontor.reporting.balance / kontor.reporting.trial (ADR-008)."
  [request]
  (cond-> {}
