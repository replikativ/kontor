(ns kontor.provider.einvoice-provider
  "EInvoiceProvider — protocol for emitting and transmitting
   e-invoice artifacts per jurisdiction (ADR-017).

   Sibling to `kontor.tax.tax-rate-provider/TaxRateProvider` (ADR-071,
   superseding ADR-005). The kernel
...
(defprotocol EInvoiceProvider
  "Per-jurisdiction e-invoice emitter / transmitter.
   ...
   Lifecycle expectation:
     1. `provider-id` / `envelope-format` return identity keywords
        (stable; used for provider dispatch + audit). W3
        added `provider-id` for uniformity with other XxxProvider
        protocols — typically the same value as `envelope-format`
        unless a single provider impl supports several formats.
  ...
  (provider-id [this]
    "A keyword identifying the implementation — :pure-xml,
     :de-xrechnung, :br-nfe, :cn-fapiao, :jp-peppol-pint, … —
     used in `:provenance` and audit logs. Per the W3 uniform shape
. Typically the same value as `envelope-format`; a
     single provider that emits several formats can return a
     distinguishing key here.")
