(ns kontor.compliance.legal-hold
  "Legal hold — write-time invariant blocking `:db/purge` of held
   ...
   BEFORE `kontor.compliance.sealing/assert-no-silent-retracts!` so the more-
   ...
   - **kontor.workflow.status-machine (ADR-034):** every state change writes
   ...
   - **kontor.compliance.audit-doc + :approval-policy (ADR-038):** both
...
;; require this ns or kontor.validation — so we can require it
;; statically without cycle.
...
(defn holds-covering
  "The inverse of `entities-held?` — return the vec of active-hold
   ...
   re-implementing scope-checking — e.g. `kontor.compliance.dsar/collect`'s
   `:legal-holds`."
  [db eid]
  (let [holds (active-holds db)]
    (if (empty? holds)
...
(defn assert-no-hold-violating-destructive-writes!
  "Mirror of `kontor.compliance.sealing/assert-no-silent-retracts!`. Throws
      ...
      (throw (ex-info "Refused: destructive write blocked by active legal hold"
                      {:type        :kontor.legal-hold/purge-blocked
                       ...
                       "Each violating eid is under an active legal
                        ...
                        kontor.compliance.legal-hold/release!) — if the hold
...
(defn place-tx-data
  "Pure tx-data builder for `place!` — entity-map construction
   ...
   `kontor.workflow.process` step. Takes the same opts as `place!` minus the
...
(defn release-tx-data
  "Pure tx-data builder for `release!` (ADR-068). Use as a
   `kontor.workflow.process` step; `release!` is the standalone wrapper."
