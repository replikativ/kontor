...
The serialized gate rejects recognizable destructive and replacement forms from the original tx-data. The writer governor independently compares protected facts in `db-before` and `db-after`, including each fact's originating transaction id. This second check is essential because Datahike's resolved report for `:db/purge` currently contains neither a purge sentinel nor the removed datoms. It is deliberately authoritative despite scanning protected resource history; a future Datahike report that exposes destructive deltas can make the same proof incremental without changing Kontor's contract.
