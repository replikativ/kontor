...
The 296 existing call sites are **not migrated**: `kontor.actor/resolve-uid-refs` normalises them in the gate, so each uid becomes one canonical actor and repeat writes converge on the same entity. Unknown uids are *provisioned* (tagged `:kontor.actor/kind :unregistered`, so an auditor can tell the enrolled from the incidental) — unless the consumer installs `:kontor.approval-policy/rule :requires-actor`, after which an unregistered actor and an unattributed seal are both **refused**. `:requires-actor` specifically is off by default: a kernel cannot decide a consumer's control environment. And `:no-self-approval` now fails **CLOSED** — a nil actor or nil creator refuses the transition, because separation of duties that cannot be verified must not be assumed. (ADR-150)

### 4.2b Attribution is mandatory where a control reads it — ADR-153

**Which approval policies are on by default is a split, not a single answer**, and stating it as one is what let ADR-150's correct verdict ship as a P0. `kontor.core/install-schema!` seeds `:no-self-approval` on kernel **compliance** edges in *every* kontor database — legal-hold release, audit-doc privilege waiver, DSAR fulfillment. **Business** modules are not seeded by the kernel: `kontor-hr` (payroll approval), `kontor-asset` (disposal), `kontor-lease` (termination) and `kontor-expense` (report approval) each seed their own with their module schema, and `kontor-procurement` seeds none.

A fail-closed rule on a seeded edge is only safe if the entity behind that edge always records a creator. Three did not — an audit-doc created via the documented minimal call, a DSAR request filed without naming the intake person, and *every* payroll run, whose builder had no actor slot at all — so those transitions were refused unconditionally. The fix is not to loosen the rule; it is to make the creator **non-nullable** at each builder that a seeded policy gates (`:uploaded-by-uid`, `:changed-by-uid`, `:actor` are now required arguments rather than optional ones), and to give `:payroll-run` an approval path at all — the seeded policy on `:computed → :approved` was unreachable dead data, which is precisely why the suite was green.

Found alongside, and the same shape of defect: `check-policy` compared a **raw** `:changed-by-uid` against a **resolved** creator eid. ADR-150 deliberately made the actor slot ergonomic — `"sarah"`, a lookup-ref, a uuid and an eid are all accepted — but `(= 4711 "sarah")` is false, so an approver who identified themselves the documented way approved their own work and the rule reported no violation. The policy check runs in the pure builder, before the gate normalises anything, so it now resolves both sides itself.

Two supporting pieces:

- **Attribution is not retractable.** `datahike.db.transaction/retract-entity` retracts every ref datom *pointing at* the entity, so `[:db/retractEntity <actor>]` — an ordinary leaver cleanup — nils `:kontor.audit/create-uid` on everything that actor created. Under a permissive rule that defeats four-eyes on all of their documents; under the fail-closed rule it strands them. So the gate (`kontor.actor/assert-attribution-preserved!`) and the writer (`kontor.governance/attribution-violations`) both refuse retraction of `:kontor.audit/create-uid` and `:kontor.actor/uid` unless the same `[e a]` is re-asserted in the same delta — which distinguishes a deletion from an ordinary cardinality-one update, and keeps deactivation (`:kontor.actor/active false`) as the modelled path.
- **A cutover, not an exemption.** `:kontor.approval-policy/effective-from` (`:db.type/instant`, deliberately *not* part of the `:kontor.approval-policy/identity` tuple — datahike cannot extend a tuple on an existing db) scopes a policy to transitions at or after a date. Books with pre-attribution history set it to go-live; `kontor.actor/backfill-create-uid!` stamps the old rows in one auditable commit pointing at an actor marked `:kontor.actor/kind :unregistered`, so reconstructed attribution stays distinguishable from recorded attribution. No `:legacy` flag and no nil-creator exemption: either would put "is the creator knowable?" inside the control, where any future write could claim it too.
...
**A reversal is the original modulo sign — and `posting-option-keys` is the contract (ADR-170).** `reverse-tx-data` used to rebuild each reversing leg from a hand-kept pull spec plus a hand-kept `cond->`, so any attribute neither list named was silently dropped. That recurred four times with a different key each time (`:ledger`, `:period-tag`, `:analytic-distributions`, `:actor`) — the same allowlist-rebuild the read side fixed in ADR-124 — and twice it was a P0: a reversal that dropped `:period-tag` landed in `:normal`, which both left the adjustment period un-netted AND slipped past a HARD SEAL on `:adjustment-13` (`closed-periods-covering` matches on `(= tag period-tag)`); a reversal that dropped `:analytic-distributions` was REFUSED by the gate on an account carrying `:kontor.account/required-analytic-plans`, making such an entry permanently uncorrectable under ADR-007's reversal-plus-re-posting rule.

The mirror is now DERIVED from `kontor.book.build/posting-option-keys` and reconciled at namespace load: `kontor.book` refuses to load if the facade admits a posting option the reversal cannot carry. A reversal **inherits the original's `:period-tag`** — the tag is half of an entry's period coordinate, coequal with the effective-date — with an explicit `:period-tag` option on `reverse!` to place a correction in a different layer (distinguished by `contains?`, so `:period-tag :normal` is expressible and is never a silent default). Analytic distributions and dimensions are mirrored as **fresh entities**, never shared: `:kontor.analytic-distribution/posting` is a cardinality-one back-ref, and two sealed documents must not share mutable state. The percent is *not* negated — the sign lives on the posting amount, and `kontor.posting.validate` requires a percent in [0,100]. `:narration` and `:display-type` joined `posting-option-keys` (both were fetched by the reversal's pull spec, re-emitted by nothing, and writable by no facade caller, so a `:tax` leg came back re-stamped `:product`). The contract is pinned by a round-trip property over `d/datoms` (`kontor.book.reversal-contract-test`) plus a pure `.cljc` canary that requires every admitted option to change `build-input`'s output (`kontor.book.build-test`, JVM + Node).
...
The attributes are part of the aggregate Kontor schema, but the resource ledger, journal, source, and sink are deliberately opt-in via `kontor.resource/install-defaults!`. An ordinary accounting book therefore does not acquire control accounts or change journal selection merely because it upgrades Kontor. A cohabiting application that does not need the full accounting kernel uses `kontor.resource/install!`: it installs the public `kontor.schema/resource` posting spine, registers the mandatory Datahike governor, and seeds the control accounts. Dvergr and other resource-managing applications therefore get the complete enforcement boundary without importing hundreds of unrelated domain attributes.
...
Resource topology and established receipts are semantically sealed. Once an account, transfer, posting, journal, ledger, or used commodity participates in resource history, its Kontor-owned facts cannot be removed, replaced, or extended; corrections are new compensating resource transfers. In particular, a fresh posting cannot be attached to an established receipt to add another coordinate under its old idempotency key. This prevents a purge, state change, route rewrite, account-kind rewrite, or semantic append from manufacturing authority after the original balance check. Consumer-owned cardinality-many metadata may still be appended.

The serialized gate normalizes entity maps and every sequential Datahike operation form, including same-transaction lookup references. This makes pure builders genuinely composable: opening a child wallet and granting its initial authority is one atomic write. The writer governor validates ordinary retractions and semantic additions from the resolved datom delta, so unrelated application writes remain O(delta). Datahike supplies the writer predicate with internal `:datahike/tx-ops`, including operations expanded by transaction functions; only a history-purge operation triggers the broader `db-before`/`db-after` history comparison, including each fact's originating transaction id. This provenance is stripped before public transaction reports cross the writer boundary. It is a required substrate contract: a predicate input without `:datahike/tx-ops` fails closed because purge cannot otherwise be distinguished soundly.

Pure `transfer-tx-data` builders accept a top-level string `:tx-tempid`, following the ordinary posting builder's composition contract. Distinct tempids allow several grants to be concatenated into one transaction for atomic fan-out. Transfer idempotency compares route, vector, actor, and any caller-supplied effective/posting timestamps; timestamps generated by the effectful wrapper are excluded from retry equality so concurrent identical commands still converge.
