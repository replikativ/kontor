...
Resource topology and established receipts are append-only. Once an account, transfer, posting, journal, ledger, or used commodity participates in resource history, existing facts on that entity cannot be removed or replaced; corrections are new compensating resource transfers. This prevents a purge, state change, route rewrite, or account-kind rewrite from manufacturing authority after the original balance check. New metadata may still be appended when it does not replace a fact.

The serialized gate normalizes entity maps and every sequential Datahike operation form, including same-transaction lookup references. This makes pure builders genuinely composable: opening a child wallet and granting its initial authority is one atomic write. The writer governor validates ordinary retractions from the resolved datom delta, so unrelated application writes remain O(delta). Datahike reports additionally expose `:tx-ops`, including operations expanded by transaction functions; only a history-purge operation triggers the broader `db-before`/`db-after` history comparison, including each fact's originating transaction id. Reports from older Datahike versions have no operation provenance and conservatively retain the full audit.
