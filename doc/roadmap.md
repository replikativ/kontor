...
- [x] **Stage K — kontor-procurement** — Full P2P + reverse flow. ADR-042 design (commit `a08c239`) + 4 implementation commits: schema/seeds (`35fcf12`), forward flow + bridge polymorphism (`*forward-commit*`), reverse flow + credit-memo (`2e570d8`), drop-ship + `:order-item-assoc` (`*assoc-commit*`). 708 tests / 2554 assertions baseline. Review-after agents (code-review + market-pain delta) pending.
