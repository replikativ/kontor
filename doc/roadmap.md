...
- [x] **Stage R — kontor-hr + per-country PayrollProvider adapters** — **Stage R complete: C1 substrate + 11 country adapters all landed.** Substrate (C1, ADR-075): `:person` + `:employment` + `:compensation` + multi-cardinality `:compensation-component` + `:pay-period` + `:payroll-run` in `kontor-hr`, plus kernel attrs `:audit-doc/category` + `:retention-policy/category` + `:audit-doc/language`, plus `kontor.payroll-provider` protocol trio (`PayrollComputeProvider` + `PayrollPostingBuilder` + `PayrollEmitProvider`) + `kontor.hr.payroll/run-payroll!` orchestrator threading `:ledgers-map` + `:state-allocations`. C2 DE (ADR-076) DATEV LODAS + HGB §249 PTO. C3 US (ADR-077) ADP GLI + multi-state via `:analytic-account/state` + ASC 710 + 401(k) match. C4 CA (ADR-078) Ceridian + ADP-CA + Wagepoint + T4/T619 + PD7A + ROE. **C4.1 QC (ADR-087)** RL-1 + RL-1 Summary + TPZ-1015. C5 FR (ADR-079) Silae + Sage + DSN NEODES. C6 AU (ADR-080) Xero + MYOB + STP Phase 2 + SuperStream + 8-state allocation. C7 BR (ADR-081) RH Sistemas + Senior HCM + Pluxee + eSocial S-1000..S-2399 + three CPC-33 accruals. C8 MX (ADR-082) CONTPAQi + Aspel NOI + CFDI Nómina v1.2 + aguinaldo. C9 IN (ADR-083) Keka + GreytHR + ZenHR + Form 24Q TDS + EPFO ECR + ESIC + per-state PT. C10 JP (ADR-084) freee + Money Forward + Yayoi + Gensen + My Number PII discipline. C11 CN (ADR-085) Yonyou + Kingdee + Beisen + 五险一金 + 34-province + 年终奖 special method. C12 AT (ADR-086) BMD + RZL + ELDA mBGM + L16 + Urlaubsrückstellung. Plus McComb-aligned substrate seams: ADR-090 generalized `:concept-iri` across 6 entity types + ADR-091 `kontor.explain` graph walks + ADR-092 `kontor.event-bus` in-process pub-sub. Research-before notes 79 + 81-84 + 87-89; review-after notes 85 + 86 (4 P0s + 5 P1s + 2 P2s closed). Cross-stage validation: trans-national Jane Doe employed across DE/US/CA simultaneously, 3 currencies, 3 payroll-runs (`test/kontor/stage_r_cross_stage_test.clj`). UK deferred per note 78 iXBRL gate; any follow-on country is a CSV-mapping + ADR add.
