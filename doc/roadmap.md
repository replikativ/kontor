...
- [ ] **Stage J-2 — Cross-cutting primitive ADRs** (per ADR-037 + research note 13). Resolves ~30 P1 items in 4 coherent passes rather than 7 punch-list patches. Each ADR is small (~50-150 LOC + schema + tests + docs) and lands as its own commit:
  - **ADR-038 Audit + Governance** — `:status-history/reason` as keyword + `:reason-note` string + `:status-history/supporting-doc` ref + `:approval-policy` companion with SoD enforcement. Resolves SOX / GDPR / auditor-codified-reasons gaps surfaced by all 5 research agents.
  - **ADR-039 Master-data** — `:partner-merge` non-destructive link + `:bank-account` entity + `:partner-bank-account` junction + `:partner/credit-limit` + `:partner-tag` segmentation + KYC/sanctions hooks. Resolves the MDM gaps from the partner-pain agent.
  - **ADR-040 Jurisdiction** — `:partner-tax-id` junction + `:invoice-line/reverse-charge?` + `:invoice/tax-inclusive?` + `:invoice-line/recognition :direct \| :deferred` + withholding-tax routing + invoice clearance states (`:pending-attestation`, `:rejected`). Resolves multi-jurisdiction gaps from the invoicing-pain agent; forward-compat for kontor-revrec.
  - **ADR-041 Workflow extensions** — time-based transitions via `:schedule` integration + side-effect intent rows + bulk-transition API + inverse-pair role-direction documentation + debit/credit map → data table. Resolves workflow + procurement-forward-compat gaps.
- [ ] **Stage K — kontor-procurement** — `:order/type :purchase` shape + requirement (requisition) entity + 3-way match (PO ↔ receipt ↔ invoice) + RTV flow. Builds on Stage J-2 primitives. (~2-3 weeks.) **Entry conditions**: ADR-038, ADR-040 (recognition keyword + multi-tax-id + reverse-charge), and ADR-041 (debit/credit data table + inverse-pair semantics) land first. `cancel-order!` reservation-release lands as part of ADR-041.
