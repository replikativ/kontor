...
extension, or maintaining an existing transactor. It assumes you have
[doc/architecture.md](architecture.md) open for the layer cake and
[doc/decisions.md](decisions.md) within reach for the *why*.
...
state machine" `kontor.workflow.state-machine` is a sibling — it governs
...
Datahike's tempid allocator IS order-stable (covered by the
...
`kontor.tax.tax-rate-provider/TaxRateProvider` (one method:
...
(ADR-005, ns removed in 2026 per ADR-071) — rate determination is
now pure data, and a `TaxFacts` is composable into a
`kontor.process` step without a side-effect to the rate engine
inside the writer lock.

### `FxRateProvider` (ADR-072)

`kontor.fx.fx-rate-provider/FxRateProvider` (`get-rate`,
...
```clojure
(require '[kontor.fx.fx-rate-provider :as fxp]
         '[kontor.fx :as fx])
...
`kontor.provider.payroll-provider` exposes three protocols that together
...
(`kontor-people-record`, the `kontor.compliance.dsar` bundler, your MCP
...
Both are kernel-enforced in `kontor.workflow.status-machine/check-policy`,
...
Two places where the "every `!` routes through the gate" rule has
an exception, each for a structural reason:

1. **`kontor.period/close!`'s `:period/lock-tx` denorm** — records
...
2. **`kontor.posting/build-transaction`'s embedded `kbt/with-vt`** —
...
- The Beancount import loader (`kontor.import-.beancount/load-into!`).
...
`test/kontor/workflow/process_test.clj/speculative-db-eid-round-trips-to-final-commit`
...
- **ADR-067 + ADR-068** for the canonical design of the transact
  ...
  `FxRateProvider`, `kontor.provider.consolidation`, `CrossTxRouter`).
  ADR-075 for the payroll provider trio. ADR-090..092 for the
  McComb-aligned substrate seams (`:concept-iri`,
  `kontor.explain`, `kontor.workflow.event-bus`). ADR-094 for the consent +
  retention + AI Act posture. ADR-113 for the fiscal-unit substrate.
...
License: Apache 2.0. Patches welcome at
