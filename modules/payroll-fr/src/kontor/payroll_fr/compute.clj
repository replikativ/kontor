(ns kontor.payroll-fr.compute
  "FR payroll compute providers — file-ingest CSV adapters for the
   dominant French engines (Silae / Sage / Cegid) plus a partner-
   program-gated API skeleton.
   ...
   (Silae / Sage Paie / Cegid Paie / ADP Streamline France) is
   authoritative for gross-to-net: cotisations URSSAF, CSG/CRDS,
   ...
   - `SilaeGlProvider` — reference CSV adapter. Silae is the dominant
     SMB engine in France (per public market share data, used by
     ~30% of French expert-comptable firms). Per-customer column
     variation is handled via a `:column-mapping` config map (the
     Silae GL export's column names are fixed in their default
     'modèle écriture' but customers customise — a `:column-mapping`
     hook lets the adapter accept variants without code change).

   - `SageGlProvider` — same shape (CSV with per-customer columns).
     Sage Paie & RH ships an 'écritures comptables' export that is
     functionally identical to Silae's pay-element CSV — only the
     column names + default code system differ. Same parser, different
     `:column-mapping`.

   - `CegidApiProvider` — partner-program-gated skeleton. Cegid Paie
     ships a REST API for partner integration; consumers with
     enrolled API credentials wire their OAuth client through
  ...
  (compute-payroll [_ _ctx]
    (throw
     (ex-info
      ...
      {:partner-program "Cegid Partner Program"
       :provider :cegid-paie
