(ns kontor.payroll-fr.wage-types
  "FR-specific `:component-kind` extensions per ADR-079 (Stage R C5).
   Open-set per ADR-071 P2-71-2 + ADR-075's `PayrollFacts` opaque-
   component contract.
   ...
   ## PCG account-tag vocabulary (note 86 §2.4 + ADR-079)
   ...
   ## DSN rubrique mapping (note 79 §C5)
...
(def standard-component-kinds
  ...
  {;; ──────────────────────────────────────────────────────────────
   ...
   ;; In-band model per note 86 P2-86-3 CA pattern: engine emits the
