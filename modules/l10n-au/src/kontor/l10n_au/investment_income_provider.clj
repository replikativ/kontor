(ns kontor.l10n-au.investment-income-provider
  "AU investment-income tax provider — `PeriodTaxProvider` (ADR-099)
   ...
   ## 45-day at-risk holding-period rule
...
). Each component is a feeder: `:liability` = NEGATIVE
...
(def kinds
  "Closed set of holder kinds the AU investment-income provider services.
   Shares the enum with `au-cgt-provider`."
...
(def trust-kinds
  "Closed set of `:tax-unit :trust-kind` values.
...
(defn- franking-credit-refundability
  "Per s.207-45 / s.207-150 ITAA 1997 +.4 — return the
   ...
   `:discretionary-fte` raises `:not-yet-implemented` (v1 scope).
  ...
  (case kind
    ...
    :trust       (case (or (:trust-kind tax-unit) :discretionary-no-fte)
                   ...
                   :discretionary-fte      (throw
                                            (ex-info
                                             "AU investment-income: :trust-kind :discretionary-fte not yet implemented"
                                             {:tax-unit tax-unit})))
...
(defn- apply-holding-period-gate
  "Apply the s.160APHO 45-day rule per event after the s.160APHT small-
   shareholder exemption check. Mutates `:franking-credit`
...
(defn- build-component
  ...
  (let [fate-franking (franking-credit-refundability kind tax-unit)
        ...
        ;; cap; aggregate AU-tax cap is consumer-side5).
        ;; When the holder is foreign-resident the foreign-source income
        ;; itself isn't taxable in AU (out of scope here —
        ...
        ;; account. We emit the SIGNAL here; the
