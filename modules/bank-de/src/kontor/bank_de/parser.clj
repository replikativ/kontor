(ns kontor.bank-de.parser
  "German bank-statement CSV configs + categorizer.
   ...
   `kontor.banking.bank-csv`; this namespace just supplies
   the per-bank `bank-configs` map, a `detect-bank` heuristic,
   and an auto-categorizer for German-language transactions
   (adapted from the author's prior MIT-licensed beleg project,
   Copyright (c) 2023-2025 replikativ).
