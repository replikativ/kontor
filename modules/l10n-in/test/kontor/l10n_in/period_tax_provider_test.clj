(ns kontor.l10n-in.period-tax-provider-test
  "Stage 1 of the tax-completion program — Indian
   personal income tax, the deliberate dual-regime stress case. The
   §87A rebate and the surcharge are base-aware adjustment items
   (ADR-099 addendum 4), so these tests run the
...
(deftest credits-and-surtaxes-are-structured
  ;; Per ADR-099 addendum 4 — §87A and the surcharge are now
...
(deftest dual-regime-election
  ...
  ;;: they cannot be one :elect schedule over a shared base.
  (let [conn (core/create-test-db)
        _    (d/transact conn
