  ...
  (let [conn (core/create-test-db)]
    ...
    (d/transact conn
                [{:kontor.journal/code "INV" :kontor.journal/name "Sales"
                 ...
                 {:kontor.period/start apr-1-2024
                  :kontor.period/end   apr-1-2025
                  :kontor.period/tag   :normal
                  :kontor.period/name  "FY24-25"}])
    conn))
...
(deftest fiscal-year-close-rolls-pnl-to-220900
  (testing "Close FY24-25 → P&L accounts zero on Apr 1 2025,
    ...
    (let [conn (bootstrap)]
      (seed-fy-24-25! conn)
      (let [db (d/db conn)
            period-eid (d/q '[:find ?p . :where [?p :kontor.period/name "FY24-25"]] db)
...
(deftest cannot-close-period-twice
  (let [conn (bootstrap)]
    (seed-fy-24-25! conn)
    (let [period-eid (d/q '[:find ?p . :where [?p :kontor.period/name "FY24-25"]] (d/db conn))]
      (in-closing/close-in-fiscal-year! conn
                                        {:period-eid period-eid
...
(deftest auto-creates-close-journal
  (testing "If the CLOSE journal isn't pre-seeded, close-in-fiscal-
    ...
    (let [conn (core/create-test-db)
          ...
          period-eid (d/q '[:find ?p . :where [?p :kontor.period/name "FY24-25"]] (d/db conn))]
...
(deftest missing-retained-earnings-throws
  (testing "If the IN chart isn't installed (no 220900), the planner
    ...
    (let [conn (core/create-test-db)]
      (v/install-invariants! conn)
      (d/transact conn [{:kontor.period/start apr-1-2024
                         :kontor.period/end apr-1-2025
                         :kontor.period/tag :normal
                         :kontor.period/name "FY24-25"}])
      (let [period-eid (d/q '[:find ?p . :where [?p :kontor.period/name "FY24-25"]] (d/db conn))]
