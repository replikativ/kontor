(ns kontor.l10n-in.investment-income-provider
   ...
   *withholds*. The recipient's tax-side aggregates BOTH withheld and
   ...
   Per.1.3 the additional 10 % flat on dividend income
   ...
   The consumer wires:
     ...
     2. `in-cgt-provider` — capital gains (separate flow).
...
(defn- count-above-threshold
  "Count keys in `source-map` whose Money amount exceeds `threshold`.
   Used for audit-trail TDS line items"
...
(defn register!
  "Register IN investment-income compute-fns with `kontor.tax.statute`.
...
(defn- resident-interest-component
  "Interest income (resident individual): folds via
   ...
   applied via `kontor.tax.statute/apply-provisions` (gates on regime +
