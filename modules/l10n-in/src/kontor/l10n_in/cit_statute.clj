(ns kontor.l10n-in.cit-statute
  "IN corporate income tax — Regular CIT + MAT (§115JB) encoded as
   `kontor.tax.statute` data per ADR-101 / ADR-101 Addendum 1.

   First end-to-end consumer of `kontor.tax.statute/compose-greater-of`
   ...
   `incometaxindia.gov.in` (CBDT) — both public-domain."
...
(def parameters
  ...
  [;; ----- Standard regime base rates -----
   ...
   ;; ----- Cess (shared between IN PIT and IN CIT —.3 / §2.1) -----
...
;; `:tax-unit :regime` and computes accordingly.
...
(def provisions
  "IN CIT statutory provisions encoded for the `kontor.tax.statute`
  ...
  [;; --------------------------------------------------------------------
   ...
   ;; PY 2023-24 turnover —.2 P1-A pre-`:tax-unit` fact, no
