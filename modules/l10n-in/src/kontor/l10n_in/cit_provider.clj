(ns kontor.l10n-in.cit-provider
  "IN corporate income tax provider — Regular CIT + MAT (§115JB)
   composed via `kontor.tax.statute/compose-greater-of` per ADR-101
   ...
   1. Reads IN CIT rates from `:parameter` data via
      `kontor.tax.statute/parameter-value-at` (rates) and
      `kontor.tax.statute/parameter-brackets-at` (surcharge bands).
...
.4.2). When absent or
   ...
   waits on the kontor carry primitive; this v1 only records the
   would-be carry on the audit trail.
   ...
   provision in vain. See research.5."
...
(defn register!
  "Register the four IN CIT compute-fns with `kontor.tax.statute`. Called
...
(defn- annotate-mat-credit
  "When MAT prevailed, stamp `:provenance :mat-credit-carry-forward`
   ...
   credit's utilisation in later AYs waits on the kontor carry
   primitive — v1 only records it on the audit trail."
...
(defn in-cit-provider
  "Build an IN CIT provider. Statute lives in `:provision` /
   ...
   `kontor.tax.statute/compose-greater-of`.
