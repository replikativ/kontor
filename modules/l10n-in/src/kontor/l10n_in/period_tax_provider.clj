(ns kontor.l10n-in.period-tax-provider
  "Indian personal income tax — as a kontor `PeriodTaxProvider`
   ...
   ## Design — regime as an explicit constructor input
...
). It is NOT an `:elect` schedule: `:elect` applies
   its sub-schedules to ONE shared base (`kontor.tax.tax-schedule` ns
   docstring), but the two Indian regimes do not
   share a base — the new regime's base is gross − ₹75,000, the old
   regime's is gross − ₹50,000 − Σ(Chapter VI-A). The election is a
   different-base choice, and `kontor.tax.tax-schedule`'s own docstring
   ...
   See the §87A rebate handling below for the strain this still leaves.
   ...
   not just the tax. Per ADR-099 addendum 4 the adjustment layer is
   base-aware, so §87A is a
  ...
  (:require [kontor.tax.period-tax-provider :as ptp]
            [kontor.tax.personal-income-tax :as pit]
...
(def ^:private cess-adjustment
 ...
 — a surtax of 4 % of the running tax (income tax − §87A
...
(defn- regime-adjustments
  "Build the adjustment layer for a regime: the
...
(defrecord InIncomeTaxProvider [inner regime]
  ...
  (period-tax-facts [_ context]
    ...
    ;;. A regime election can also ride `context
...
(defn in-income-tax-provider
  "An IN personal income tax `PeriodTaxProvider`. India runs two
   ...
   check can; see the Stage-1 abstraction audit.
