(ns kontor.l10n-mx.closing
  "Mexican fiscal-year close — rolls P&L accounts to retained
   ...
   Sits as a thin country wrapper over `kontor.reporting.closing/close-
   fiscal-year!`, pinning:
   ...
   - **`plan-mx-fiscal-year-close-tx-data`** — pure planner that
     ...
     in one `kontor.workflow.process`.
...
(defn- account-by-code [db code]
  (kacct/resolve-code db code {:context "MX year-end closing"}))
