  ...
  "kontor-payroll-mx — Mexican CFDI Nómina v1.2 payroll adapter (ADR-082).
   ...
   This module composes three substrate layers:
     ...
     1. **Engine provider** — protocol `MxEngineProvider` —
        ...
        CSV) and returns a vector of `:payroll-facts` per period:
        the canonical
        ...
        `kontor.provider.payroll-provider/PayrollComputeProvider`, which is
     ...
     3. **CFDI emitter** — `MxCfdiEmitter` (impl
        ...
        `kontor.provider.payroll-provider/PayrollEmitProvider`, satisfied by
   ...
   - `kontor.workflow.process` ADR-067 composition: `import-period!` is one
...
;; kontor.provider.payroll-provider/PayrollComputeProvider. An MxEngineProvider
