...
.
   ...
   Reference:(wage-type vocabulary), §4 (CoA wage-account
...
(def standard-component-kinds
  ...
  {;; ──────────────────────────────────────────────────────────────
   ...
   ;; treatment. The IIT engine reads
...
(defn account-tag
  "Return the :account-tag keyword for a kind, or nil. Consumer's
   :accounts map keys on this tag. Per."
...
(defn assert-known!
  "Throw ex-info on any unknown kinds in `components`; return the input
   on success. Matches CA's `validate-catalog` posture ( — the
