(ns kontor.payroll-cn.e2e-test
  "Stage R C11 — end-to-end Yonyou CSV → kontor `run-payroll!` test.
   ...
   `kontor.hr.payroll/run-payroll!` → `kontor.workflow.process/run-process` →
   `kontor.validation/transact-with-validation`.

   Per.2: multi-province wage allocation in the GL uses
  ...
  (:require [clojure.string :as str]
            ...
            [kontor.provider.payroll-provider :as pp-proto])
