(ns kontor.payroll-br.compute
  "BR payroll compute providers — file-ingest CSV adapters for BR
   payroll engines per ADR-081 §2 (RH Sistemas, Senior HCM, Pluxee).
   The engines converge on a wage-type-per-row CSV / GL-export shape.
   ...
   - `RhSistemasGlProvider` — reference CSV adapter with configurable
     column mapping. BR payroll engines tend to follow the
   ...
   - `PluxeeCsvGlProvider` — Pluxee ships a slightly different layout
     — column-position based rather than header-based. Same parser
     core with a position-based row→component mapping.
...
;; ============================================================================
;;
;; Pluxee exports a position-based CSV (no headers; column position
;; dictates meaning):
;;   col 0  CNPJ
;;   col 1  CPF (employee)
;;   col 2  matricula
...
(defn parse-pluxee-csv
  "Parse a Pluxee position-based CSV (string or Reader). The default
   field-separator is ';' (Pluxee's convention); pass `:separator \\,`
   to override.
