(ns kontor.l10n-fr.cgt-provider-test
  "Tests for the FR CGT providers (corporate + personal) over the
   ...
   the three worked examples from research,
...
(deftest statute-installs-2026-PS-rates
  (testing "the LFSS 2026 PS rate bump (17.2 % → 18.6 %) splits by income class"
    (let [conn (fresh)
          db   (d/db conn)]
    ...
    (let [conn (fresh)]
      ...
      (let [facts (run-personal conn "INDIV" p2026
        ...
        (is (== 62800M (amt (:liability mob)))
            "Total = €25 600 IR + €37 200 PS = €62 800")
        (is (= :fr-pfu (:regime mob)))))))

(deftest mobiliere-2025-disposal-PS-186-retroactive
  (testing "2025 mobilière disposal pays PS at 18.6 % (LFSS 2026 rétroactif aux revenus du patrimoine 2025)"
    (let [conn (fresh)]
      ...
      (let [facts (run-personal conn "INDIV" p2025
        ...
        ;; Total = €31 400. Under the buggy encoding pre- fix, PS would
...
;; ============================================================================
;; §3. Mobilière — barème + abattement renforcé 85 %
;; ============================================================================
    ...
    (let [conn (fresh)]
      ...
      (let [facts (run-personal conn "INDIV" p2026
        ...
        (is (== 35000M (-> (filter #(= :mob-ir-base (:line %)) (:line-items mob))
                           first :value :amount))
            "≥8y under général → 65 % abattement")))))
...
;; ============================================================================
;; §6. Immobilière — long-held secondary residence
;; ============================================================================
    ...
    (let [conn (fresh)]
      ...
      (let [facts (run-personal conn "INDIV" p2026)
        ...
        (is (== 170100M (-> (filter #(= :immo-ps-base (:line %)) (:line-items immo))
                            first :value :amount))
            "23y → 37 % PS abattement → PS base = €270 000 × 63 % = €170 100")
...
(deftest pro-long-terme-238-quindecies-standard-600k-not-fully-exempt
  (testing "§238 quindecies STANDARD non-agricultural transmission €600k → partial taxable"
    (let [conn (fresh)]
      ...
      (let [facts (run-personal conn "INDIV" p2026
        ...
        (is (== 100000M (-> (filter #(= :pro-lt-base (:line %)) (:line-items plt))
                            first :value :amount))
            "taxable base = €500k × 20 % = €100k (standard cliffs unchanged)")
        (is (== 30000M (amt (:liability plt)))
            "€12 800 IR + €17 200 PS = €30 000 on €100k taxable base")))))

;; ============================================================================
;; §9. Titres de participation — IS-side QPFC 12 %
;; ============================================================================
...
(deftest §238-quindecies-bitemporal-cliff
  (testing "standard cliff stable €500k; agricultural cliff bitemporal €500k → €700k"
    (let [conn (fresh)
          db   (d/db conn)]
