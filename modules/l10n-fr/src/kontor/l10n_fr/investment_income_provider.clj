(ns kontor.l10n-fr.investment-income-provider
  "FR investment-income tax providers — TWO `PeriodTaxProvider`s
   (ADR-099) over the ADR-101 statute-as-data substrate.

   FR has FIVE overlapping shapes for investment income — the v1
   provider covers the four with kernel-substrate fit:
   ...
   - **PFU / barème** (CGI Art. 200 A + 117 quater) — default 31.4 %
     ...
     component (per ADR-099.4 recommendation b).
...
(defn- dividendes-component
  ...
  (let [{:keys [commodity authority db]} opts
        ...
        ;; CSG deductible carry-forward to N+1
