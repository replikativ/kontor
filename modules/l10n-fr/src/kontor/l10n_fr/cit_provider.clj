(ns kontor.l10n-fr.cit-provider
  "FR corporate income tax provider — Impôt sur les sociétés (IS) +
   ...
   the statute-as-data substrate (ADR-101).
   ...
   2. For the single IS component sets `:component :is` in ctx, calls
      `kontor.tax.statute/apply-provisions` for each relevant concept
   ...
   - Déficit reportable (CGI Art. 209 I al. 3; €1 M + 50 % cap) — this
     is an inter-period carry frontier; FR IS is the demand-trigger
     but the substrate work is out of scope here.
...
(defn register!
  "Register the three FR CIT compute-fns with `kontor.tax.statute`. Called
