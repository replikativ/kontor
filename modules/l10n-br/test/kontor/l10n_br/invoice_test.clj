...
(deftest intra-state-sp-goods-invoice-posts
  (testing "SP → SP goods: R$1000 net.
    ...
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-SP-1"
...
(deftest inter-state-sp-to-ba-goods-invoice-posts
  (testing "SP → BA goods (S/SE → N/NE/MW): R$1000 net.
    ...
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-SP-BA-1"
                   :invoice/issue-date jan-15
                   :invoice/from-state "SP"
                   :invoice/to-state "BA"
...
(deftest inter-state-rj-to-rs-goods-different-states
  (testing "Inter-state RJ (S/SE) → RS (S/SE) = 12% interstate.
    ...
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-RJ-RS-1"
                   :invoice/issue-date jan-15
                   :invoice/from-state "RJ"
                   :invoice/to-state "RS"
                   :invoice/lines
                   [{:invoice-line/quantity 1
...
(deftest manufactured-goods-with-ipi-posts
  (testing "SP → SP manufactured goods (NCM-tied IPI 10%): R$1000 net.
              ...
              Gross = 1381.43"
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-MFG-1"
                   :invoice/issue-date jan-15
                   :invoice/from-state "SP"
                   :invoice/to-state "SP"
                   :invoice/lines
...
(deftest services-invoice-iss-only-posts
  (testing "Pure services R$1000 net, ISS 5% (São Paulo).
              ...
              not Goods revenue."
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-SVC-1"
                   :invoice/issue-date jan-15
                   :invoice/lines
                   [{:invoice-line/quantity 1
                     :invoice-line/unit-price 1000M
...
(deftest services-invoice-min-iss-rate-2-pct
  (testing "ISS 2% (low-tax municipality, e.g. Barueri/SP).
              ISS 2% × 1000 = 20.00."
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-SVC-2"
                   :invoice/issue-date jan-15
                   :invoice/lines
                   [{:invoice-line/quantity 1
                     :invoice-line/unit-price 1000M
                     :invoice-line/tax-classification :services
                     :invoice-line/iss-rate 0.02M}]}]
      (inv/post-br-invoice! conn inv-map)
      (let [db (d/db conn)]
    ...
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-EXP-1"
                   ...
                   [{:invoice-line/quantity 1
                     ...
                     :invoice-line/tax-classification :export}]}]
      (inv/post-br-invoice! conn inv-map)
      (let [db (d/db conn)]
...
(deftest cash-sale-debits-caixa-not-ar
  (testing ":invoice/cash-sale? true → debit Caixa (1.01.01.01.01)
...
(deftest difal-can-route-to-its-own-account
  (testing "Override :codes :difal-code so DIFAL posts to a different
    ...
    (let [conn (bootstrap)
          inv-map {:invoice/external-id "INV-DIFAL-1"
                   :invoice/issue-date jan-15
                   :invoice/from-state "SP"
                   :invoice/to-state "BA"
                   :invoice/lines
    ...
    (doseq [{:keys [name lines from to]}
      ...
      (let [conn (bootstrap)
            inv-map (cond-> {:invoice/external-id (str "INV-Z-" name)
                             :invoice/issue-date jan-15
                             :invoice/lines lines}
                      from (assoc :invoice/from-state from)
                      to   (assoc :invoice/to-state to))]
        (inv/post-br-invoice! conn inv-map)
        (let [db (d/db conn)
