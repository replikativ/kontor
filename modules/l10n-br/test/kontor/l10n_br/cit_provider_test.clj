(ns kontor.l10n-br.cit-provider-test
  "BR corporate income tax provider tests — ADR-101 substrate's second
   ...
   `kontor.tax.statute/apply-provisions` fold) computes real BR CIT
   against blueprint.
   ...
   Coverage map:
     ...
     §6   Regime-scope regression () — Real vs Presumido isolation
...
;; ============================================================================
;; §1. Lucro Real clean profit — Example A
;; ============================================================================

(deftest lucro-real-clean-profit-example-a
  (testing "Lucro Real annual, R$ 800k book profit, no adjustments —.A"
  ...
  (testing "Lucro Real annual with multas + doações + CSLL-addback + dividendos
    ...
    (let [facts (compute {:book-profit                  800000M
      ...
      (testing "CSLL liability: 555000 × 9 % = 49950"
        ...
        (is (empty? (:surtaxes csll))))
      (testing "Total: 120725 + 49950 = 170675 — matches.B"
        (is (== 170675M (total-liability facts)))))))

;; ============================================================================
;; §3. Lucro Presumido services (Example C,.C)
;; ============================================================================

(deftest lucro-presumido-services-example-c
  (testing "Lucro Presumido trimestral Q4, serviços 32 % presunção —.C"
    (let [facts (compute {:regime :br-lucro-presumido :atividade-codigo :servicos}
      ...
      (testing "CSLL: 1030k × 9 % = 92700; no surtax"
        ...
        (is (empty? (:surtaxes csll))))
      (testing "Total Q4 federal: 251500 + 92700 = 344200 — matches.C"
...
;; ============================================================================
;; §5. Bitemporal bank CSLL — Lei 14.183/2021 sunset (Example D,.D)
;; ============================================================================
    ...
    (let [conn (core/create-test-db)]
      ...
      (let [db (d/db conn)
            ...
            n-regimes (count (d/q '[:find ?r :where [?r :kontor.regime/code _]
                                  ...
                                  db))]
        ;; Final counts: 15 provisions, 17 parameters.
        (is (= 17 n-params))
        (is (= 15 n-provs))
        (is (= 3  n-regimes))))))
