(ns kontor.l10n-br.cgt-provider-test
  "Tests for the BR CGT provider (ADR-102 + ADR-101, research note 130).

   Worked examples §2 of note 130:
...
;; ============================================================================
;; §3. Lane A — art. 39 residence-reinvestment exemption (note 130 §2 Example A)
;; ============================================================================
  ...
  (testing "R$ 1.2M sale w/ R$ 900k re-invested → 75% exempt → R$ 15 000 tax"
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :individual p2026)
        ...
        (is (== 15000M (-> cmp :liability :amount))
            "100k × 15 % bracket I = R$ 15 000 (note 130 §2 Example A)")))))
...
(deftest swing-isenção-under-R20k-monthly-cap
  (testing "single swing sale at R$ 18k/month → exempt (note 130 §2 Example B)"
    (let [conn (fresh)]
...
;; ============================================================================
;; §6. Lane B-day — 20 % rate, NO isenção (note 130 §2 Example B)
;; ============================================================================
  ...
  (testing "day-trade gain → flat 20 %, even at small amounts"
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :individual p2026)
        ...
        ;; 12k × 20% = 2400 (note 130 §2 Example B)
...
;; ============================================================================
;; §10. Mixed-month worked example B (note 130 §2)
;; ============================================================================
