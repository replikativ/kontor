(ns kontor.l10n-br.cit-provider
  "BR corporate income tax provider — IRPJ (15 % + 10 % adicional) +
   ...
   (ADR-101; research notes 121 + 162). Mirrors
   ...
   `BR-IRPJ-adicional-10pct`, regime-agnostic per note 162 §2.3),
   ...
   even though Receita Federal collects both (note 162 §5.2). Symmetric
   ...
   CA also need it, promote to `kontor.statute` (P1-4 in note 162 §4)."
...
(defn- months-in-period
  "Approximate the number of months in `:period`. 1 / 3 / 12 cover the
   ...
   bisextile-aware month arithmetic, promote to `kontor.statute`."
  [period]
...
(defn register!
  "Register the BR CIT compute-fns with `kontor.statute`. Called
