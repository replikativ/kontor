(ns kontor.l10n-br.period-tax-provider
  "Brazilian personal income tax — Imposto sobre a Renda da Pessoa
   Física (IRPF) — as a kontor `PeriodTaxProvider` (ADR-099).
   ...
   `kontor.tax.period-tax-provider/balance` then yields the saldo a pagar
...
."
  (:require [kontor.tax.personal-income-tax :as pit]
...
(defn br-irpf-provider
  "BR personal income tax — IRPF — `PeriodTaxProvider`. The 5-band
   ...
   consumer supplies it; `kontor.tax.period-tax-provider/balance` then
