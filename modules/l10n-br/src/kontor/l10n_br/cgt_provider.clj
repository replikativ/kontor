(ns kontor.l10n-br.cgt-provider
  "BR capital-gains tax provider — `PeriodTaxProvider` (ADR-099) over
   ...
   - **Individual** — three lanes, each compartmentalised:
     ...
     (note 130 §4 gap B option 2).

   - **Corporation** — net gain folds into IRPJ + CSLL via
     `:cit-base-additions` (`:tax-unit :tax-regime :lucro-real`).
     `:lucro-presumido` raises informative error in v1 (note 130
     §1.3 different presumption base).

   ## PL 1087/2025 (Lei 15.270/2025) high-earner minimum tax

   Per note 130 §1.5, the new 10 % IRPFM minimum tax effective
   ...
   The four-bracket PF ladder is annual (note 130 §5.2 — Receita
   walks the ladder against the year-to-date gain running-total per
   ...
   consumer without disposals wires `empty-source` and gets zero
   components."
...
(def lanes
  "The closed set of BR CGT lanes a disposal classifies into. Mirrors
   note 130 §3.3 + ADR-102 `:loss-bucket` convention."
...
;; Which BR asset-classes land in which lane. Open vocabulary (note 130
;; §3.2) — the kernel ships these mappings; a consumer can extend by
...
(def default-asset-class->lane
  ...
  {:br-real-estate-residencial :br-ganho-capital
   ...
   :br-listed-equity-swing     :br-renda-variavel-long  ; alias from note 130
...
(def lucro-regimes
  "PJ tax regime enum carried in `:tax-unit :tax-regime`. v1 supports
   ...
   (different presumption base — note 130 §1.3)."
...
(defn- classify
  "Classify one disposal into a BR CGT lane.
   ...
   rollover/proceeds)`. Per note 130 §2 Example A: a R$ 1.2M sale
...
;; ============================================================================
;; Monthly aggregate isenções — provider-side fold (note 130 §4 gap A)
;; ============================================================================
...
(defn- pequeno-valor-eligible?
  "Per note 130 §1.1.1, the R$ 35k pequeno-valor isenção (Lei 9.250/95
    ...
    (let [db          (or (:db ctx)
          ...
          (case kind
            ...
            (let [;; --- Lane A: pequeno-valor R$ 35k folds across :br-equity-comum
                  ...
                  ;; (note 130 §1.1.1 — small-share sales) participates in
                  ;; the R$ 35k aggregate.
              ...
              (when (= regime :lucro-presumido)
                (throw (ex-info
                        (str "BR CGT v1 supports only :lucro-real. "
                             ...
                             "base (note 130 §1.3) — file an extension "
...
(defn br-individual-cgt-provider
  "Build a BR individual CGT provider. Required: `:source` — a
   `DisposalSource` (kernel protocol). Optional:
