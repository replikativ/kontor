(ns kontor.l10n-at.investment-income-provider
  "AT investment-income tax providers — `PeriodTaxProvider` (ADR-099)
   over the ADR-101 statute-as-data substrate. Research note 154.
   ...
   - `at-corporate-investment-income-provider` (`:kind :corporation`) —
     ...
     `§10 foreign-corp guard` (note 146 §3.2): when
   ...
   ## §10 INVERSION direction (note 134 §6.1 + note 146 §3.3 P0-3 fix)
   ...
   The within-year CGT-losses ↔ investment-income netting (note 154
   §1.6 / §3.3) is an ORCHESTRATOR-level concern across the AT KESt
   CGT provider + this provider. v1 does NOT implement the cross-
   provider netting — the consumer wires the two providers
   independently and the within-year netting is an explicit follow-up
   (per the orchestrator pattern in note 154 §5.3). Both providers
   ...
   `:investment-income-event` substrate per note 153 Gap 1 is not yet
...
(def ^:private §10-classifications
  "Closed set of §10 KStG classifications a corporate-dividend event
   may carry (note 154 §1.7). `:domestic` and `:foreign-portfolio` /
...
(defn- §10-default-exempt-component
  "§10 KStG default-exempt branch — domestic / foreign-portfolio /
   ...
   Per BFG 2024 (note 154 §1.10): when §10 exempts the dividend, NO
...
(defrecord ATKestInvestmentIncomeProvider
  ...
  (period-tax-facts [_ {:keys [entity period inputs] :as ctx}]
    (let [db        (or (:db ctx)
          ...
          ;; off from the 27.5 % bucket per note 154 §1.6).
...
(defrecord ATCorporateInvestmentIncomeProvider
  ...
  (period-tax-facts [_ {:keys [entity period inputs] :as ctx}]
    (let [db        (or (:db ctx)
          ...
          ;; Foreign-corp guard (per AT CGT P0-2, note 146 §3.2): when
...
(defn at-corporate-investment-income-provider
  "Build an AT corporate investment-income provider (`:kind :corporation`).
   ...
   Foreign-corp guard (note 146 §3.2): `:tax-unit
