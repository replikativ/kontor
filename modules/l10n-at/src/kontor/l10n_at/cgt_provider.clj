(ns kontor.l10n-at.cgt-provider
  "AT capital-gains tax providers — `PeriodTaxProvider` (ADR-099) over
   ...
   AT CGT splits across THREE provider records:
   ...
   - **`at-kest-cgt-provider`** (kind `:individual-kest`) — §27/§27a
     ...
     carryforward (Jan 1 reset —.3). Bank-withheld via
   ...
   - **`at-immoest-provider`** (kind `:individual-immoest`) —
     ...
     against §28-Vermietung income (first cross-category CGT loss in
     the kontor substrate; provider writes to
     ...
     **Umwidmungszuschlag** (§30 Abs 6a EStG, BBG 2025, effective
     ...
     30 % of the positive land-slice gain (cap
     `Bemessungsgrundlage = min(1.30 × Gewinn ; Erlös)` enforced).

   - **`at-corporate-cgt-provider`** (kind `:corporation`) — §10 KStG.
     **INVERSION** of the usual exemption-flag direction: default is
     TAX-NEUTRAL for qualifying Schachtelbeteiligung
     ...
     **Foreign-corp guard** (§10 Abs 2 KStG,.2): §10
   ...
   buckets:
     :at-kest       — DELIBERATELY UNUSED (resets Jan 1;.3)
     :at-immoest    — only the §30 Abs 7 mechanism applies; tracked
                      ...
                      {year-keyword amount}` shape.

   ## DisposalProvider

   The provider depends on the kernel `DisposalProvider` protocol
   (`src/kontor/disposal_provider.clj`); a consumer without disposals
   wires `kontor.provider.disposal-provider/empty-provider` and gets zero
   components."
...
(def kest-asset-classes
  "Closed set of `:kontor.disposal/asset-class` keywords the KESt provider
   ...
   bank deposits is not a disposal event —"
...
(def ^:private hauptwohnsitz-flags
  "The two `:exemption-claimed` flags that trigger
   Hauptwohnsitzbefreiung (alternative tests, either one suffices)."
...
(defn- §10-qualifying?
  "True iff the disposal qualifies as a §10 KStG internationale
   Schachtelbeteiligung: foreign corp AND ≥ 10 %
   ownership AND ≥ 1-year holding.

   Foreign-corp gate: §10 Abs 2 KStG applies to
...
(defn- residence-component
  "Hauptwohnsitzbefreiung short-circuit — emit a zero-tax component
   for audit. The component records the flag that
...
(defn- neuvermoegen-component
  "Standard Neuvermögen ImmoESt component — 30 % on (proceeds − basis).
   ...
   when the Hauptwohnsitz flag is absent (exemption must be explicitly
   claimed). When `:at-herstellerbefreiung` is in the exemption set,
   the consumer must have set `:kontor.disposal/basis-amount` to
   represent the LAND-only portion (building share exempt; the
   deviation is documented in `:notes`)."
...
(defn- altvermoegen-component
  "Altvermögen pauschale component — 4.2 % (unwidmet) or 18 %
   (gewidmet) of GROSS PROCEEDS. The deemed gain is
...
(defn- §30-loss-carry-component
  "§30 Abs 7 EStG loss-distribution component — the first cross-
   category CGT loss in the kontor substrate.
   ...
   Substrate convention: the destination is
...
. The gain that landed in the GL as
...
(defn- §10-option-taxable-component
  "§10 KStG with Option zur Steuerwirksamkeit elected — gain enters
   CIT base at the standard rate. No exemption; the
...
(defn- §10-option-loss-component
  "§10 KStG with Option AND a LOSS — §12 Abs 3 Z 2 KStG
   Siebentelregelung: the loss is spread over 7 years.
               ...
               (mapv (fn [d]
                       (let [g (realized-gain d)
                         ...
                         (cond
                           ...
                           ;; back to the CIT base. Note 146 §5:
...
(defn at-immoest-provider
  "Build an AT ImmoESt provider — §30/§30a EStG on real-estate
   ...
   Required: `:source` — a `DisposalProvider`.
   Optional:
