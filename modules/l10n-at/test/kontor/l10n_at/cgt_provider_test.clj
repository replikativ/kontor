(ns kontor.l10n-at.cgt-provider-test
  "Tests for the AT CGT providers (ADR-102 + ADR-101).
  ...
  (let [conn (core/create-test-db)]
    ...
    (d/transact conn [{:kontor.commodity/symbol "EUR" :kontor.commodity/name "Euro"
                      ...
                      {:kontor.entity/code "HOLDCO" :kontor.entity/name "HoldCo"
                       :kontor.entity/kind :company :kontor.entity/country "AT"
...
(deftest kest-shares-flat-27-5
  (testing "Frau Huber sells OMV shares for €56k after €38k basis"
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :kest p2026)
        ...
        ;; 18,000 × 27.5% = 4,950 — exact match to.1
...
(deftest hauptwohnsitzbefreiung-5of10-zero-tax
  (testing "Herr Mayer Vienna apartment — 5-of-10 → €0 tax"
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :immoest p2026)
        ...
        (is (== 0M (-> cmp :liability :amount))
            "Hauptwohnsitzbefreiung (5-of-10) → zero ImmoESt (matches.2)")
...
;; ============================================================================
;; §6b. §30 Abs 6a Umwidmungszuschlag (BBG 2025)
;; ============================================================================

(deftest umwidmungszuschlag-post-2025-07-01-bauland-disposal
  (testing "Brandauer example: €180k LAND-slice gain on rezoned Bauland → surcharge component at 30% × ImmoESt rate"
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :immoest p2026)
        ...
        (is (== 16200M (-> surch :gross-liability :amount))
            "surcharge tax = 30 % × surcharge base = €16 200")
...
(deftest umwidmungszuschlag-rate-effective-from-2025-07-01
  (testing "the surcharge parameter is loaded as date-keyed (BBG 2025)"
    (let [conn (fresh)
          db   (d/db conn)
          rate-2026 (kontor.tax.statute/parameter-value-at
                     db "AT.EStG.§30-Abs-6a.umwidmungszuschlag-rate"
                     #inst "2026-06-01")
          rate-pre  (kontor.tax.statute/parameter-value-at
                     db "AT.EStG.§30-Abs-6a.umwidmungszuschlag-rate"
                     #inst "2025-01-01")]
...
(deftest §10-default-exempt-loss-neutralizes-gl
  (testing ": default-exempt LOSS branch emits POSITIVE :cit-base-additions to neutralize the GL loss (not the signed-negative gain which would DOUBLE the loss)"
    (let [conn (fresh)]
...
(deftest §10-domestic-participation-not-inverted
  (testing ": AT-domestic 25 % GmbH stake disposal — :tax-unit :held-entity-domestic? true blocks the INVERSION → no component (gain stays in CIT base)"
    (let [conn (fresh)]
...
(deftest §10-default-exempt-no-option
  (testing "Müller-Holding 25 % Swiss AG gain — default exempt, no Option"
    (let [conn (fresh)]
...
(deftest §10-option-elected-gain-taxable
  (testing "Müller-Holding 25 % Swiss AG — Option elected, gain taxable"
    (let [conn (fresh)]
...
(deftest §10-option-loss-siebentelregelung
  (testing "Option elected + LOSS — 1/7 spread"
    (let [conn (fresh)]
...
(deftest cit-rate-ladder-2024-23-percent
  (testing "CIT rate 23 % active for 2024+ (ÖkoStRefG 2022 stepped reduction)"
    (let [conn  (fresh)
          ...
          rate-2023 (kontor.tax.statute/parameter-value-at
                     db "AT.KStG.cit-rate" #inst "2023-06-01")
          rate-2022 (kontor.tax.statute/parameter-value-at
