(ns kontor.l10n-at.investment-income-provider-test
  "Tests for the AT investment-income providers (ADR-099 + ADR-101).
   ...
   - §1 KESt 27.5 % on dividends (Endbesteuerung) — Frau Huber
   - §2 KESt 25 % on Sparbuch interest — Frau Huber
   ...
   - §5 DBA-Quellensteuer credit — Apple US dividend
   - §6 §10 Abs 1 Z 1 KStG domestic dividend exempt
   - §7 §10 Abs 1 Z 5-6 foreign-portfolio exempt + BFG 2024 lost credit
   - §8 §10 Abs 3 Option zur Steuerwirksamkeit → fully taxable
   - §9 Foreign-corp guard — blocks §10 INVERSION
   - §10 §10 Abs 4 switch-over — low-tax jurisdiction
...
;; ============================================================================
;; §1. KESt 27.5 % on dividends — Frau Huber
;; ============================================================================
...
;; ============================================================================
;; §4. Regelbesteuerungsoption — §27a Abs 5 EStG
;; ============================================================================
...
;; ============================================================================
;; §9. Foreign-corp guard (,.2) — blocks §10 INVERSION on domestic stakes
;; ============================================================================

(deftest §10-foreign-corp-guard-blocks-inversion-for-domestic-stake
  (testing ": :held-entity-domestic? true blocks §10 INVERSION on foreign-portfolio/schachtel"
    (let [conn  (fresh)
          ;; Without the guard, the provider would silently route €50k
          ;; to :cit-base-deductions, understating CIT by 23 % × €50k = €11,500
          ;; on a dividend that should actually be taxable.
          facts (run-corporate conn
                               {:corporate-dividend-events
                                [{:gross 50000M
                                  :§10-classification :schachtelbeteiligung
...
(deftest §27-abs-8-cross-event-type-netting-is-orchestrator-concern
  (testing "Within-year netting between CGT losses and II income is documented as orchestrator scope"
