(ns kontor.payroll-ca.e2e-test
  "End-to-end bilingual flow3 (the C4 acceptance
  ...
  (let [conn (bootstrap)
        ...
        report (payroll/run-payroll!
                conn {:pay-period pp-eid
                      ...
                      ;; (otherwise the wall-clock-default makes the
                      ;; PD7A period query exclude the postings once
                      ;; the calendar rolls past 2026-06-01).
    ...
    (testing "Emit provider produced one audit-doc with :payroll-filing category"
      ;; Note 86 — canonical vocabulary; was :payroll.
      (let [docs (d/q '[:find [?e ...]
