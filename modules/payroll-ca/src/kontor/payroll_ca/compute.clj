(ns kontor.payroll-ca.compute
  "CA payroll compute providers — file-ingest CSV adapters for CA
   payroll engines plus a Wagepoint API skeleton.
   ...
   it into `PayrollFacts` per `kontor.provider.payroll-provider`.

   ## Provider trio

   - `CaPayrollCsvProvider` — reference CA payroll CSV adapter.
     Per-customer column variation handled via a `:column-mapping`
     config map.

   - `AdpCanadaProvider` — same GLI 10-column shape as US; the only
     difference is the pay-element code vocabulary (CPP not SS, EI
     not FUTA). The `:ca-mode? true` config flag is informational —
     the actual switch is the `:pay-element-codes` lookup table.
   ...
   - `WagepointApiProvider` — skeleton + TODO. Wagepoint's
     ...
     their OAuth credential through `:credentials`.
...
;; ============================================================================
;; CaPayrollCsvProvider — CSV with configurable column mapping
;; ============================================================================

(defn- ca-csv-row->component
  "One CSV row → one component map. Returns nil if the row is a
   pay-group header / blank row. The column layout is per-customer-
   configurable — the adapter takes a `:column-mapping` opts map for
  ...
  (let [emp-col (or (:employee-id-col column-mapping) "employee-external-id")
    ...
    (when (and (not (str/blank? emp))
      ...
      (let [debit  (coerce-bigdec (get row dr-col))
            ...
            ;; A CSV pay-element row carries either a
...
(defn parse-ca-csv
  "Parse a CA payroll GL CSV (string or Reader) into a vector of
   `{:employee-external-id :kind :amount :employer-side?}` maps. Drops
   the ADP-style 'balancing row' (applies here too because some
   engines emit it under some configs) — the parser verifies the
   per-employee net amount sums to zero before discarding any
   balancer row.

   Rows mapped to `:__skip-payable` are dropped silently; these are
   the engine's pre-balanced payable-mirror rows (e.g. CPP-ER-PAY /
   NET-WAGES variants) that kontor's posting builder derives on its
   own from the employer-side expense components."
  [source opts]
  (->> (read-csv-rows source)
       (mapv #(ca-csv-row->component % opts))
       (remove nil?)
...
(defrecord CaPayrollCsvProvider [opts]
  ...
  (compute-payroll [_ ctx]
    (let [{:keys [csv-source column-mapping pay-element-codes
      ...
      (when-not source
        (throw (ex-info "CaPayrollCsvProvider needs :csv-source" {})))
      (when-not ext->eid
        (throw (ex-info "CaPayrollCsvProvider needs :external-id->eid" {})))
      (when-not codes
        (throw (ex-info "CaPayrollCsvProvider needs :pay-element-codes" {})))
      (let [parsed (parse-ca-csv source {:column-mapping mapping
...
;; ============================================================================
;;
;; ADP General Ledger Interface is a fixed-shape CSV:
;;   col 0  client-code
;;   col 1  gl-account
;;   col 2  description
;;   col 3  fiscal-date
...
;;   col 9  run-id
;;
;; ADP's "balancing row" trap: the parser MUST verify
...
;; Default header names per the ADP GLI format (documentation
...
(defn parse-adp-csv
  "Parse an ADP GLI CSV (string or Reader) into a vector of component
   maps. Verifies per-employee net = 0 before returning (the engine's
   balancer row makes the file always balance; if the file doesn't,
   the engine emitted bad data + the consumer's payroll run is wrong
   upstream).

   `:headerless?` true skips the header check (the engine sometimes
   emits no header)."
...
(defrecord WagepointApiProvider [opts]
  ...
  (compute-payroll [_ _ctx]
    (throw
     (ex-info
      "WagepointApiProvider is a skeleton. Wagepoint API access is partner-program-gated; supply an enrolled OAuth credential + live HTTP client implementation. See.1."
