(ns kontor.payroll-ca.chart
  "Installer for the CA payroll account-tag set + (optional) starter
   accounts. Follows the same shape as kontor.l10n-ca.chart but layers
   on top — consumers run kontor.l10n-ca.chart/install! first for the
   base CA chart, then this for the payroll extension.

   Reference: note 84 §7.2."
  (:require [clojure.edn :as edn]
            [clojure.java.io :as io]
            [datahike.api :as d]))

(defn load-starter
  "Load the payroll-extension starter chart (note 84 §7.2)."
  []
  (-> "kontor/payroll_ca/coa_starter.edn" io/resource slurp edn/read-string))
...
(defn install!
  "Idempotent install of payroll tags + the starter chart on top of an
   already-installed l10n-ca/chart. Safe to skip the accounts and
