(ns kontor.payroll-ca.pd7a
  "PD7A remittance helper. NOT an emitter — kontor does NOT file a
   PD7A because PD7A is CRA-to-employer correspondence. What kontor
   DOES do is total the three statutory CRA payable buckets (Income
   Tax / CPP / EI) for a remittance period so the consumer can remit
   via My Payment / online banking / paper voucher and record the
   matching cash payment.

   ## Remitter type → schedule
...
(def ^:private remitter-schedules
  "Per-remitter-type schedule metadata. See.2."
...
(defn- next-business-day-offset
  "Naive business-day offset — skips Sat/Sun, ignores statutory
   holidays. Per.4 #11 the proper holiday-aware version
...
(defn next-due-date
  "Compute the next remittance due date given a remitter type + period
   ...
   shifts are a follow-up. For :accel-t1 we
  ...
  "Compute the three statutory CRA payables owed for a remittance
   ...
   Optional:
     :rp-account-tag string — when supplied, filters postings to those
                              tagged with the RP routing tag.
