...
- **Order → invoice construction** (`make-invoice-from-order!`,
  ...
  match audit trail (ADR-042 P0-6).
...
- **GL account resolution** (`kontor.invoice.posting/resolve-gl-
  account`). The OFBiz `GlAccountTypeDefault` pattern: per-(account-
  type, entity) lookup with a tenant-wide fallback, surfaced as
  `:gl-account-default/*` schema.
...
- Per-shipment invoicing (the OFBiz `invoicePerShipment` pattern;
  ship-group filtering is a documented follow-up)
