...
The OFBiz / Tryton / Workday / SAP-BP shape carried into the kernel
namespace.
...
- **Polymorphic contact mechs** — `:contact-mech` root with typed
  ...
  `:from-date` (inclusive) + `:thru-date` (exclusive — OFBiz / Tryton
  convention). Multi-purpose routing via `:partner-contact-mech-
...
- **Effective-date convention** — `active-as-of?` predicate is
  `from-date ≤ d < thru-date`. `:thru-date` is exclusive (OFBiz /
  Tryton convention). Junction-time validity is layered on top of
...
- Identity / authentication / authorization → `kontor-authz`
...
- [ADR-068](../../doc/decisions.md) — `merge-partners-tx-data`
  ...
  with-validation`
- Research note 12 — OFBiz / Tryton / Workday / SAP-BP shape
  survey that informs the design
