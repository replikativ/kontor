(ns kontor.incorporation
  "The Phase 3 keystone — `incorporate-tx-data` / `incorporate!`.
   Per ADR-103, the individual→corporation continuum: the
   ...
   1. **New entity row** — the corp's `:entity` is materialised in
      ...
      ADR-074 saga and lives in `kontor.workflow.side-effect.cross`).
   ...
   3. **Founder's Investment-in-NewCo holding** — debit the
      founder's `Investment:<NewCo>` account at BASIS; credit the
      contributed asset accounts on the founder's side.
      THIS IS KEY: the founder's basis in the new shares is
   ...
   - **Shape B (cross-DB)** — founder personal DB and corp DB are
     ...
     wraps ADR-074 `kontor.workflow.side-effect.cross`) is the saga primitive.
     Deferred to a future stage; cross-DB sagas exist for other
     consumers (ADR-074) so the precedent is set.
   ...
   See ADR-095 (kontor.book verb facade)."
...
(defn- founder-side-postings
  "Build the founder-side leg: Dr Investment-in-NewCo at BASIS;
   Cr the founder's contributed asset accounts at their basis (NBV);
   Dr the founder's liability accounts (for liabilities the corp
   assumed)."
...
(defn incorporate-tx-data
  "Pure tx-data builder for `incorporate!` (single-DB Shape A).
   Returns a tx-data vector covering:
