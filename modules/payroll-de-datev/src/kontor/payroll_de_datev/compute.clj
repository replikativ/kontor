(ns kontor.payroll-de-datev.compute
   ...
   **Lohn-Buchungsbeleg** (Report 80) which exports in the *same*
   ...
   `:kontor.payroll-run/state :buchungsbeleg-invalid` (4
   gotcha 12).
...
;; Verrechnungskonto code per chart variant.
...
(defn- row->posting
  "Project one EXTF data row to a flat posting map (key data only).
   Column ordinals2 / kontor.l10n-de.datev §1."
  [year cols]
...
;; ============================================================================
;; PayrollFact assembly
;; ============================================================================
;;
;; The Bruttomethode book-keeping pattern:
;;
...
(defn- assert-verrechnung-balanced!
  "Per.2 / §11.18 invariant: after all postings for a
...
(defn group->payroll-fact
  "Build one PayrollFact from a vector of EXTF-projected posting maps
   ...
   invariant on the original rows, and aggregates
  ...
  (let [kinds-map (account-kinds-for coa)
        ...
        ;; the Verrechnung sits on. Each row is a
        ;; two-line booking:
