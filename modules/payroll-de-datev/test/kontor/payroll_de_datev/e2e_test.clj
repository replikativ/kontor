...
1), creates a :person + :employment + :pay-period,
...
(def ^:private skr04-payroll-accounts
  "The 10 load-bearing SKR04 payroll accounts. We seed
...
(deftest e2e-de-gmbh-monthly-payroll-round-trip
  (let [conn (bootstrap)
        ...
        ;; lookup-ref resolution at transact time —.4 gotcha).
        ...
        run (d/pull db
                    ...
                    run-eid)
        ;; Note 86 fix: substrate orchestrator now links
