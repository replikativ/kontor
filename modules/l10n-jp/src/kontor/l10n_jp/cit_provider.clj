(ns kontor.l10n-jp.cit-provider
  "JP corporate income tax provider — 法人税 + 地方法人税 + 防衛特別法人税 +
   ...
   (ADR-101 / ADR-101 Addendum 1). Mirrors the DE
   ...
   3. Threads `:national-cit-amount` into the inhabitants' ctx so the
      ...
      component's gross — cross-component surtax wired manually in
      the provider, audit trail in `:provenance`.
...
(defn- jp-inhabitant-per-capita
  "法人住民税 均等割 — fixed per-capita levy from the 10-cell
   capital-class × headcount-class table — the substrate carries the
   levy as a surtax-on-zero adjustment item
...
(defn- jp-cit-sme-large-income-schedule
  "Note 125 — the SME-with-income-over-¥1B 2-bracket schedule
   ...
   `kontor.tax.tax-schedule/apply-schedule` with `[base ctx]` after
   `kontor.tax.statute/resolve-schedule-template` resolves the
...
(defn register!
  "Register JP CIT compute-fns with `kontor.tax.statute`. Called
...
(defn- inhabitant-default-schedule
  "Inhabitants' component default schedule — flat 0 %. The component
   ...
   adjustment-side surtax items in the fold — 'surtax on zero' rather
   than introducing a `:fixed-amount` schedule kind."
  []
...
(defn- component-items
  "For one component (`:national` / `:enterprise` / `:inhabitant`),
   ...
   `:book-profit` already net of book→tax add-backs (the add-back
   menu is open-ended and a Phase 4 deliverable; see ADR-106
...
;; ============================================================================
;; Pro-forma 外形標準課税 helpers
;;
;; The provider runs THREE prefectural-side passes for large corporations
;; (consumer-driven via :tax-unit :is-sme? false): the existing income-base
;; plus the value-added base + capital base. SMEs continue to get the single
;; income-base pass (today's behaviour).
;;
;; Value-added formula per 地方税法 §72-14:
;;   収益配分額         = 報酬給与額 + 純支払利子 + 純支払賃借料
;;   雇用安定控除       = if payroll / 収益配分額 > 70%
;;                         then max(0, 報酬給与額 − 収益配分額 × 70%)
;;                         else 0                          (§72-20)
;;   付加価値額         = max(0, 収益配分額 + 単年度損益 − 雇用安定控除)
;;
;; The value-added compute is provider-internal logic (not a `:provision`)
;; because it composes four `:inputs` slots; the provision side just swaps
;; the schedule to 1.26% via `:op :schedule-override`. The four buckets
;; are `:inputs`-only in v1 — consumer pre-computes.
;; ============================================================================

(defn- ^java.math.BigDecimal jp-value-added-base
  "Compute 付加価値額 from the four `:inputs` buckets + the two
   `:JP.Enterprise.employment-stability-deduction-{trigger,coefficient}`
   parameters. Floors at 0 per §72-14 (3). Returns BigDecimal.

   `:inputs` keys (caller pre-computes; provider does not marginalize
   the GL):
     :jp/value-added-payroll          — 報酬給与額 (gross wages)
     :jp/value-added-net-interest     — 純支払利子 (interest paid − received)
     :jp/value-added-net-rent         — 純支払賃借料 (rent paid − received)
     :jp/value-added-single-year-pl   — 単年度損益 (pre-NOL income;
                                        defaults to `:book-profit`)"
  [db ctx as-of]
  (let [inputs    (:inputs ctx)
        zero      0M
        payroll   (or (:jp/value-added-payroll        inputs) zero)
        net-int   (or (:jp/value-added-net-interest   inputs) zero)
        net-rent  (or (:jp/value-added-net-rent       inputs) zero)
        single-pl (or (:jp/value-added-single-year-pl inputs)
                      (:book-profit                   inputs)
                      zero)
        sdr       (.add ^java.math.BigDecimal (.add ^java.math.BigDecimal payroll net-int) net-rent)
        ;; 雇用安定控除: only fires when payroll / SDR > trigger
        trigger   (statute/parameter-value-at db "JP.Enterprise.employment-stability-deduction-trigger" as-of)
        coef      (statute/parameter-value-at db "JP.Enterprise.employment-stability-deduction-coefficient" as-of)
        sdr-pos?  (pos? (compare sdr zero))
        deduction (if (and sdr-pos?
                           (pos? (compare (.divide ^java.math.BigDecimal payroll sdr 10 java.math.RoundingMode/HALF_UP)
                                          trigger)))
                    (let [threshold (.multiply ^java.math.BigDecimal sdr coef)
                          raw       (.subtract ^java.math.BigDecimal payroll threshold)]
                      (if (pos? (compare raw zero)) raw zero))
                    zero)
        raw-base  (.subtract ^java.math.BigDecimal
                             (.add ^java.math.BigDecimal sdr single-pl)
                             deduction)]
    (if (pos? (compare raw-base zero)) raw-base zero)))

(defn- ^java.math.BigDecimal jp-capital-base
  "Read paid-in capital + capital reserves at `:as-of` from ctx. Per
   Q4.1 `:as-of` is overrideable and defaults to
   `:period :to`. v1 trusts the consumer's `:tax-unit :paid-in-capital`
   + `:tax-unit :capital-reserves` (令和 6 anti-avoidance triage =
   v1.x —)."
  [_db ctx _as-of]
  (let [unit (:tax-unit ctx)]
    (.add ^java.math.BigDecimal
          (or (:paid-in-capital   unit) 0M)
          (or (:capital-reserves  unit) 0M))))

(defn- value-added-default-schedule
  "Default schedule for the :enterprise-value-added component — flat 0
   pre-override. The large-corp `JP-Enterprise-§72-pro-forma-value-added`
   provision overrides to 1.26 % via `:op :schedule-override`. SMEs
   never see this component (provider branches on `:is-sme?`)."
  []
  {:kontor.schedule/type :flat :rate 0M})

(defn- capital-default-schedule
  "Default schedule for the :enterprise-capital component — flat 0
   pre-override. The large-corp `JP-Enterprise-§72-pro-forma-capital`
   provision overrides to 0.525 %."
  []
  {:kontor.schedule/type :flat :rate 0M})

(defn- enterprise-value-added-component
  "Build the prefectural value-added-base component (付加価値割).
   Large-corp only — the caller `period-tax-facts` branches on
   `:is-sme?` and only invokes this when false. Note 182 §2."
  [db ctx as-of functional-commodity]
  (let [base       (jp-value-added-base db ctx as-of)
        {:keys [tax-items schedule-overrides provisions]}
        (component-items db ctx as-of :enterprise-value-added)
        scoped-ctx (assoc ctx :component :enterprise-value-added :db db :as-of as-of)
        schedule   (pick-schedule (value-added-default-schedule) schedule-overrides)
    ...
    {:kind            :corporate-income-tax
     ...
     :regime          nil
     :provenance      {:provider-id :jp-cit
                       :statute     "地方税法 §72-12 / §72-14 / §72-20 (付加価値割 + 雇用安定控除)"
                       :provisions-applied (mapv :kontor.provision/code provisions)
                       :as-of       as-of}}))

(defn- enterprise-capital-component
  "Build the prefectural capital-base component (資本割). Large-corp
   only. Snapshot `:as-of` (Q4.1 — overrideable, defaults to
   `:period :to`). Note 182 §2."
  [db ctx as-of functional-commodity]
  (let [base       (jp-capital-base db ctx as-of)
        {:keys [tax-items schedule-overrides provisions]}
        (component-items db ctx as-of :enterprise-capital)
        scoped-ctx (assoc ctx :component :enterprise-capital :db db :as-of as-of)
        schedule   (pick-schedule (capital-default-schedule) schedule-overrides)
        gross      (ts/apply-schedule schedule base scoped-ctx)
        {liability :liability tax-resolved :resolved}
        (ts/apply-adjustments gross tax-items scoped-ctx)]
    {:kind            :corporate-income-tax
...
(defn- inhabitant-component
  "Build the inhabitants' tax component map. Schedule = flat 0 %
   ...
   dependency on the national CIT component (audit trail)."
...
(defrecord JPCITProvider [id commodity]
  ...
  (period-tax-facts [_ {:keys [entity period db inputs] :as ctx}]
    (let [as-of       (or (:as-of ctx) (:to period))
          ...
          ;; per 地方税法 the 法人税割 base is the 法人税
          ...
          ;; — large corporations (外形対象法人) get
          ;; TWO additional prefectural-side components: the value-added
          ;; base (付加価値割) + the capital base (資本割). Driven by
          ;; consumer-supplied :tax-unit :is-sme? — matches the existing
          ;; income-base SME/large branch. SMEs see neither component;
          ;; their facts shape is unchanged.
          large?            (false? (get-in ctx [:tax-unit :is-sme?]))
          enterprise-va     (when large? (enterprise-value-added-component db ctx as-of commodity))
          enterprise-cap    (when large? (enterprise-capital-component   db ctx as-of commodity))
          components        (cond-> [national enterprise inhabitant]
                              enterprise-va  (conj enterprise-va)
                              enterprise-cap (conj enterprise-cap))]
