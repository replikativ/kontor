(ns kontor.l10n-jp.investment-income-provider
  "JP investment-income tax provider — `PeriodTaxProvider` (ADR-099)
   over the ADR-101 statute-as-data substrate. Research note 151.
...
(defn- jp-haitō-kōjo-national
  "Compute-fn registered with `kontor.statute`. Reads ctx for
...
(defn- jp-haitō-kōjo-jūmin
  "Compute-fn registered with `kontor.statute` for the inhabitants
   side of the 配当控除."
  ^java.math.BigDecimal [ctx]
  (let [db          (:db ctx)
        as-of       (as-of-from-ctx ctx)
...
(defn register!
  "Register JP investment-income compute-fns with `kontor.statute`.
    ...
    (let [_ (or (:db ctx)
      ...
      (case kind
        ...
        (let [classified
              ...
              ;; (statutory cascade — note 151 §6.1).
