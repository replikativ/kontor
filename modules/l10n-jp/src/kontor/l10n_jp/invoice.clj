(ns kontor.l10n-jp.invoice
  "Japan Qualified Invoice System (QIS / 適格請求書) — in force since
   ...
   ## Posting builder — ADR-071 tax abstraction
   ...
   `kontor.tax.tax-posting-builder/compute-tax-postings` and collapsed
...
;; `kontor.l10n-jp.tax-provider` (ADR-071 migration).
...
(defn- line-net
  "Per-line net amount = qty × unit-price, rounded HALF-EVEN to whole
   yen (JPY :kontor.commodity/precision 0). Tolerates :kontor.invoice-line/line-
...
(defn- revenue-code-for-class
  "Choose the revenue account for a line's JCT class. Callers can pin
   a different account via :kontor.invoice-line/account."
  [jct-class codes]
...
(defn- revenue-postings
  ...
  (let [grouped (group-by
                 (fn [line]
                   (let [jct-class (or (:kontor.invoice-line/jct-class line) :standard)
    ...
    (vec
     (for [[[acct-code _jct-class] ls] grouped
       ...
       {:kontor.posting/account   acct
        :kontor.posting/amount    (.negate net)
...
(defn- jct-postings
  "Per-invoice output-JCT postings, via the ADR-071 tax provider +
   ...
   rate-account with `kontor.tax.tax-posting-builder/aggregate-postings`."
  [db lines codes commodity-eid date]
    ...
    (tpb/aggregate-postings
     (mapcat (fn [l]
               (tpb/compute-tax-postings
                provider builder
                {:base      (line-net l)
                 :jct-class (or (:kontor.invoice-line/jct-class l) :standard)
  ...
  "Pure tx-data builder for a Japanese QIS sales invoice (ADR-068).
   ...
   Each invoice-line:
     {:kontor.invoice-line/quantity    <number-or-bigdec>      ; OR
      ...
      :kontor.invoice-line/jct-class   <keyword>               ; default :standard
                                                        ; one of
                                                        ; #{:standard
                                                        ;   :reduced
                                                        ;   :non-taxable
   ...
   Optional top-level fields:
     :kontor.invoice/cash-sale?    when true, post Dr cash (110100 default)
...
(defn validate-invoice
  ...
  (let [{:kontor.invoice/keys [external-id issue-date lines]} invoice
        line-classes (map #(or (:kontor.invoice-line/jct-class %) :standard) lines)]
    (cond-> []
      ...
      (conj {:field :kontor.invoice-line/jct-class :issue :invalid-class
