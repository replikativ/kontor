(ns kontor.l10n-jp.cgt-provider
  "JP capital-gains tax provider — `PeriodTaxProvider` (ADR-099) over
   ...
   `:jp-real-estate-long-residence`). Per
   ...
   - The 復興特別所得税 (2.1 % × the NATIONAL portion only) rides as
     ...
     `kontor.tax.statute` provision the provider invokes per-component.
   ...
   - **`:kind :corporation`** — corporate CGT folds into the CIT base
     (no separate JP regime;.3). The provider returns ONE
   ...
   consumer without disposals wires `kontor.provider.disposal-provider/empty-provider`
...
(defn register!
  "Register JP CGT compute-fns with `kontor.tax.statute`. Called at
...
(defn jp-individual-cgt-provider
  "Build a JP individual CGT provider. Required: `:source` — a
   `DisposalProvider` (kernel protocol). Per-disposal regimes are
...
(defn jp-corporate-cgt-provider
  "Build a JP corporate CGT provider. Required: `:source`. Returns a
   ...
   threads into the JP CIT provider's `:inputs` (no separate JP
   corporate CGT regime)."
  [{:keys [source id]}]
  (when-not source (throw (ex-info ":source DisposalProvider required" {})))
