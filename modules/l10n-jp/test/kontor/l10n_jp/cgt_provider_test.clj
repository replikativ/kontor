(ns kontor.l10n-jp.cgt-provider-test
  "Tests for the JP CGT provider (ADR-102 + ADR-101).

   The shape mirrors the US CGT test suite — fixture + per-§ regime
   coverage — but the substrate stress sits in different places:

   - The Jan-1 measurement rule — the JP-unique
     design feature.
   - The five separate-taxation compartments.
   - The §35 ¥30M residence deduction.
...
(deftest §35-30m-deduction-applies-to-plain-long
  (testing "§35 deduction reduces base on plain :long (no §31-3 election)"
    (let [conn (fresh)]
...
(deftest §35-deduction-applies-to-short-term
  (testing "§35 + §32 short-term residence (sub-5-year)"
    (let [conn (fresh)]
...
;; ============================================================================
;; §7. Worked examples from
;; ============================================================================

(deftest worked-example-mr-sato-short-term-real-estate
  (testing "Mr Sato: 2022-03 buy / 2026-04 sell short-term"
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :individual p2026)
        ...
        (is (== 5350050M (-> re :liability :amount))
            "Mr Sato worked example —.1")))))

(deftest worked-example-mr-takahashi-long-residence-with-§35
  (testing "Mr Takahashi: 2013-06 buy / 2026-05 sell, §35 + §31-3"
    (let [conn (fresh)]
