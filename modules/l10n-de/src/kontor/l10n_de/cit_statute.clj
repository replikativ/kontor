(ns kontor.l10n-de.cit-statute
  "DE corporate income tax — KSt + Soli + GewSt — encoded as
   `kontor.tax.statute` data per ADR-101. The first end-to-end consumer
...
(def parameters
  ...
  [{:kontor.parameter/code         "DE.KSt.rate"
   ...
   ;; one-row migration — the law's six sub-letters have been stable
   ;; since 2008-01-01 UntStRefG; the d/e/f weights came in via
...
(def provisions
  "DE CIT statutory provisions encoded for the `kontor.tax.statute`
  ...
  [;; --------------------------------------------------------------------
   ...
   ;;
   ;; CGT-CIT integration: when the DE CGT provider is
   ...
   ;; Freibetrag applied ONCE, and the universal ¼ once. Note 120
   ;; + — splitting interest/rental into separate provisions with
