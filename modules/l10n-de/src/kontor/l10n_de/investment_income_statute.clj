(ns kontor.l10n-de.investment-income-statute
  "DE investment-income tax — §20 EStG (dividends + interest) under the
   Abgeltungsteuer regime — encoded as `kontor.tax.statute` data per
   ADR-101. 

   THE KEY INSIGHT: most parameters this provider
...
(def provisions
  "DE investment-income provisions. TWO provisions: Soli + KiSt on
   ...
   Single install path — `install!` here ships everything. A consumer
   who calls only `inv-statute/install!` gets the complete tax stack.
