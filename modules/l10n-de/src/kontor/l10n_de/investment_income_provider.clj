(ns kontor.l10n-de.investment-income-provider
  "DE investment-income tax provider — `PeriodTaxProvider` (ADR-099)
   ...
   bridge fn would be `combined-§20-facts`
   ...
   S. 4 EStG — the substrate key is `:de-§20-other`.
...
(defn register!
  "Register DE investment-income compute-fns with `kontor.tax.statute`.
...
;; — single install path. The compute-fn `:de-soli-on-§20-
...
. Returns a map of `{<lane-key> <bigdec>}`.
...
(defn- §20-teileinkünfte-fold-component
  ...
  (let [db        (:db ctx)
        ...
        ;; dividends) track to the same 60 % statutory anchor.
