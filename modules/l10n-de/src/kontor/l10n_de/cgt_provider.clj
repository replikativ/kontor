(ns kontor.l10n-de.cgt-provider
  "DE capital-gains tax providers — `PeriodTaxProvider` (ADR-099) over
   ...
   DE CGT splits cleanly along the corporate /
   ...
   - **`de-corporate-cgt-provider`** (kind `:corporation`): §8b KStG
     ...
     `kontor.tax.sole-proprietor/business-income-input` (ADR-100) between
...
(def individual-loss-buckets
  "The four+1 isolated loss buckets the individual
...
(defn register!
  "Register DE CGT compute-fns with `kontor.tax.statute`. Called at
...
(defn cgt-§8b-addback-input
  "Sum the `:cit-base-additions` across all CGT components into a single
   ...
   Wire-shape recap: the corporate CGT provider emits
