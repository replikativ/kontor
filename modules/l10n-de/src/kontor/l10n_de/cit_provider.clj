(ns kontor.l10n-de.cit-provider
  "DE corporate income tax provider — KSt + Soli + GewSt — built as a
   ...
   (ADR-101). First end-to-end consumer of
   `kontor.tax.statute`; the reference example for the per-jurisdiction
   ...
   2. For each component (KSt / GewSt) sets `:component` in ctx, calls
      `kontor.tax.statute/apply-provisions` to gather base-side and tax-side
...
(defn register!
  "Register the four DE CIT compute-fns with `kontor.tax.statute`. Called
