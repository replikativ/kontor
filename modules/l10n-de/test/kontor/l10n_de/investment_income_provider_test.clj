(ns kontor.l10n-de.investment-income-provider-test
  "Tests for the DE investment-income provider (ADR-099 + ADR-101).
   ...
   - §6 §20-other carry-in netting —.6.
   - §7 Herr Weber retiree — Path A standalone."
...
;; ============================================================================
;; §1. Frau Schmidt —.1 — standalone Abgeltungsteuer + Soli
;; ============================================================================
...
(deftest de-§20-other-carry-in-reduces-base
  (testing "shared :de-§20-other carry-in is consumed"
    (let [conn  (fresh)
          facts (run-provider conn
  ...
  (testing "BY retiree, Catholic, single, k=0.08: 4400 → ~945.83"
    (let [conn  (fresh)
      ...
      (let [expected 945.83M
        ...
        (is (< diff 0.01M)
            (str "liability " total " is within 1 cent of.2's 945.83"))))))
...
;; ============================================================================
;; §8. GL-scan integration — F7 regression guard
;;
...
(deftest single-install-path-yields-full-soli-stack
  (testing "F8 regression: `inv-statute/install!` alone must ship
