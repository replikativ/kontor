  ...
  (let [conn (core/create-test-db)
    ...
    (testing "all 7 named seed rows present"
      (doseq [code ["DE-HGB257-hr-personnel"
        ...
        (is (some? (ref-eid db :retention-policy/code code))
            (str "seed row " code))))

    (testing "the hr-activity-content seed is a 0-year :purge floor"
      (let [eid (ref-eid db :retention-policy/code
                         "DE-DSGVO-hr-activity-content-floor")
            row (d/pull db '[*] eid)]
        (is (= 0 (:retention-policy/duration-years row)))
        (is (= :purge (:retention-policy/expiry-action row)))
        (is (= :hr-activity-content (:retention-policy/category row)))))

    (testing "the hr-medical seed is a 30-year archive policy"
      (let [eid (ref-eid db :retention-policy/code
                         "DE-GefStoffV-hr-medical")
            row (d/pull db '[*] eid)]
        (is (= 30 (:retention-policy/duration-years row)))
    ...
    (testing ":country/code DE exists + is referenced"
      (let [de-eid (ref-eid db :country/code "DE")
        ...
        (is (some? de-eid))
        (is (= de-eid (-> row :retention-policy/jurisdiction :db/id))))))
