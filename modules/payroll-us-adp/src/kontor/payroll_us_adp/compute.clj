(ns kontor.payroll-us-adp.compute
  "ADP General Ledger Interface (GLI) compute provider — ADR-077.
   ...
   delimited, ASCII (UTF-8-safe). The ADP GLI 10-column CSV is
   documented in the public format spec; this loader handles the
   standard layout.
   ...
   Config-driven so the same parser can handle other payroll-engine
   CSV variants with a different column-map.
   ...
   `kontor.provider.payroll-provider` contract."
...
(defn parse-gli
  "Parse an ADP GLI CSV source into a structured result map.
   ...
   zero. Per.3 + §9.4 gotcha #1: ADP files MUST sum to
...
(defrecord AdpGliComputeProvider [opts]
  ...
  (compute-payroll [_ {:keys [variable-inputs employment-eids]}]
    ...
    ;;  back-compat; canonical key is `:csv-source`
