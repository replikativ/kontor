...
(defprotocol AtFilingEmitProvider
  "AT-specific filing emitter (intentionally distinct from the kernel
   `kontor.provider.payroll-provider/PayrollEmitProvider`; THIS one returns
   ...
   Mirrors the shape of `kontor.provider.einvoice-provider/EInvoiceProvider`.
   Implementations produce wire-format bytes and records :audit-doc
   for the audit chain.

   Renamed from `PayrollEmitProvider` once the kernel
   `kontor.provider.payroll-provider/PayrollEmitProvider` landed in Stage R;
