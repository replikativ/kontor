...
(def bmd-wage-code-map
  "BMD-NTCS Lohnart-Nr → kontor wage-type keyword. BMD ships with a
   default Lohnart catalogue (numerable 0001-9999); the entries below
   are the catalogue-default codes. Customer-customized catalogues
   require a per-customer override map at the adapter boundary.

   Public reference: BMD Knowledgebase Lohnart-Übersicht — public,
   not proprietary."
  {"0001" :grundgehalt
   "0002" :überstunden
...
(def rzl-wage-code-map
  "RZL Lohn wage-type code → kontor wage-type keyword. RZL uses a
   short alpha-code convention.

   Public reference: RZL FAQ FibuExport — public."
  {"GRU" :grundgehalt
   "UST" :überstunden
