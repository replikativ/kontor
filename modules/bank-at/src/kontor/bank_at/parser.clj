(ns kontor.bank-at.parser
  "Austrian bank-statement CSV configs + categorizer.
   ...
   Generic CSV engine lives in `kontor.banking.bank-csv`. AT and
   ...
   Raiffeisen banks and Sparkassen each lightly vary their layouts):
     ...
     BANK AUSTRIA (UniCredit)
       ...
       Date: dd.MM.yyyy  •  Amount: signed German
       Bank Austria has not published an authoritative spec; treat the
       layout above as one common variant.
...
(def bank-configs
  {:erste
   ...
   ;; embedded in Buchungstext. Treat as one common variant since the
   ;; bank does not publish an authoritative spec.
   :bank-austria
   {:encoding "UTF-8" :skip-rows 0 :date-format "dd.MM.yyyy" :separator \;
    :amount-style :german
   ...
   ;; counterparty in Buchungstext, '+' prefix on credits.
   :bawag-psk
   {:encoding "UTF-8" :no-header? true :date-format "dd.MM.yyyy" :separator \;
    :amount-style :german
