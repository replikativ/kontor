(ns kontor.disposal.schema
  "kontor-disposal companion schema — ADR-102.
   ...
   - **Jurisdiction-specific extension** (sparse, populated when the
     ...
     loss-bucket. Each is optional — a generic disposal needs only
     the core.
   ...
   - **Holding-period** is BOTH `:acquired-on` + `:disposed-on` (the
     ...
     event recorded itself — the law-as-it-stood doctrine. JP uses a
     Jan-1 measurement rule as a canonical example.
   ...
   ## DisposalProvider protocol

   Kernel CGT providers depend on a `DisposalProvider` protocol (one
   method: `(disposals-facts this ctx)` per the uniform `(this ctx)`
   shape). Pure-service
   consumers don't load the companion → no `:kontor.disposal/*` schema
   → CGT providers see no disposals (return nil). Loose coupling;
   matches the existing `TaxRateProvider` / `FxRateProvider` pattern."
  ...
  [;; --- Identity ---------------------------------------------------------
   ...
   {:db/ident       :kontor.disposal/acquired-on
    ...
    :db/doc         "Acquisition date of the subject. The date the
                     ...
                     the enum `:short / :long` is not enough (DE has
                     1y/10y/tax-free-beyond cutoffs; UK indexation
                     needs per-acquisition date)."}
   ...
   {:db/ident       :kontor.disposal/disposed-on
    ...
    :db/doc         "Disposal date — the moment ownership transferred.
                     ...
                     classifier; the classifier reads the
   ...
   {:db/ident       :kontor.disposal/depreciation-taken-amount
    ...
    :db/doc         "Accumulated depreciation taken against the
                     ...
                     long-term-capital lanes (required for US
                     correctness)."}
