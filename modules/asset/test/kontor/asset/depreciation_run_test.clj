...
(deftest straight-line-plan-is-equal-periods
  (let [conn (bootstrap)
        ...
        plan (dp/plan-schedule (dp/provider-for :straight-line) (d/db conn) book)]
...
(deftest revise-book-re-plans-only-the-unfired-tail
  (let [conn (bootstrap)
    ...
    (testing "revise total useful life 120 → 60 months"
      ...
      (let [plan (dp/plan-schedule (dp/provider-for :straight-line)
                                   (d/db conn) book)
...
(deftest sum-of-years-digits-is-accelerated
  (let [conn (bootstrap)
        ...
        plan (dp/plan-schedule (dp/provider-for :sum-of-years-digits)
                               (d/db conn) book)
        amts (mapv :amount (:periods plan))]
    (is (= 60 (count amts)))
