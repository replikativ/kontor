(ns kontor.asset.depreciation-provider
  "The DepreciationProvider protocol + companion-shipped built-ins —
   ...
   Sibling of `kontor.tax.tax-rate-provider` (ADR-005) and
   `kontor.provider.costing-provider` (ADR-029): the kernel/companion ships
...
(defprotocol DepreciationProvider
 ...
 collapsed the prior `(this db book)` arity to the
   uniform `(this ctx)` shape — `ctx` is `{:db <datahike-db> :book
   <book-eid-or-pair>}`."
  ...
  (plan-schedule [provider ctx]
    "Given a ctx `{:db <datahike-db> :book <eid-or-[asset ledger]>}`,
     return the full forward plan:
...
(defn- assemble
  "Build the `:periods` vector. `starting-book-value` is the carrying
   ...
   base still threads correctly — code-review/).
    ...
    (let [{:keys [schedule depreciable-base salvage-value n-periods frequency
      ...
      (plan-result
       ...
       (assemble
        ...
        ;; `assemble` accumulator (code-review).
