(ns kontor.import-edgar.core
  "kontor-import-edgar — ingest SEC EDGAR `companyfacts` JSON into
   the kontor `:kontor.reported-fact/*` substrate.
   ...
   authoritative shape and DO NOT parse XBRL XML / iXBRL HTML — the
   JSON API is XBRL-derived and sidesteps the parser
       ...
       (let [db (d/db conn)
         ...
         (cond
           ...
           (let [{:keys [tx-data prior vt-from]} frag
                 ...
                 ;; :kontor.reported-fact/*. The atomicity limitation
                 ;; between the two transacts is documented.
                 _ (when prior
                     (let [prior-tx-eid
                           (d/q '[:find ?tx .
                                  :in $ ?e
