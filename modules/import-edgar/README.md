...
- XBRL XML / iXBRL HTML parsing — the JSON API path sidesteps
  the parser problem entirely; a future
...
- **No XBRL XML / iXBRL HTML parser.** The JSON API path is
  XBRL-derived and sidesteps the parser problem entirely.
  A future `kontor-xbrl` companion is the home for FDTA
...
Apache 2.0. **SEC EDGAR data is public domain** — no license
