(ns kontor.payroll-au.wage-types
  "AU-specific `:component-kind` extensions per ADR-080 + research
   ...
   ## Validation convention (note 86 P2-86-5)
   ...
   See note 79 §5.3 (AU = C7 of the roadmap) + the task brief
...
;; ============================================================================
;; Validation (P2-86-5 convention — throws on failure)
;; ============================================================================
...
(defn assert-valid!
  "Throws ex-info with `:errors` set to the validate output if the
   ...
   `validate-catalog` + US's `assert-valid!` (P2-86-5 — across-adapter
   consistency)."
  [extras-map]
