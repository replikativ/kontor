(ns kontor.payroll-au.compute
  "AU payroll compute providers — file-ingest CSV adapters for AU
   payroll engines (Xero Payroll AU + MYOB) plus a Reckon One skeleton.
   ...
   - `XeroGlComputeProvider` — Xero Payroll AU's GL Journal export
     (CSV). Xero's GL Journal report exposes the per-pay-period DR/CR
     breakdown including the OTE / PAYGW / Super splits. Column layout
     is per-customer-configurable so the adapter takes a
     `:column-mapping`.
   ...
   ## Canonical `:variable-inputs` keys
...
(defrecord ReckonOneComputeProvider [opts]
  ...
  (compute-payroll [_ _ctx]
    (throw
     (ex-info
      "ReckonOneComputeProvider is a skeleton. Reckon One's STP-Phase-2 GL export shape is publicly documented; wiring lands when a Reckon-using consumer surfaces. Use XeroGlComputeProvider with a custom :column-mapping for the same shape in the interim."
      {:provider :reckon-one
