(ns kontor.l10n-ca.cgt-provider
  "CA capital-gains tax provider — `PeriodTaxProvider` (ADR-099) over
   ...
   CGT has NO statutory short-vs-long distinction,
   NO own bracket schedule, and NO surtax. Every
   ...
   Per the design: this is `:inputs` shape (per-taxpayer cumulative
   ...
   :ca-superficial-loss` flag (full detection
   ...
   concern (`book/credit-cda`), NOT this provider —7."
...
(defn- component
  "Build the single `:capital-gains-tax` component. The provider
   returns ONE component per call regardless of `:kind`;
...
(defrecord CACapitalGainsTaxProvider
  ...
  (period-tax-facts [_ {:keys [entity period inputs] :as ctx}]
    ...
    (let [db          (or (:db ctx)
          ...
          ;; + personal trusts only); cap stays at 0.
...
(defn ca-individual-cgt-provider
  "Build a CA individual CGT provider. Required: `:source` —
   a `DisposalProvider` (kernel protocol)."
  [{:keys [source id]}]
