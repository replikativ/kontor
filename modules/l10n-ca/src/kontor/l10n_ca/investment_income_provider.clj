(ns kontor.l10n-ca.investment-income-provider
  "CA investment-income tax provider — `PeriodTaxProvider` (ADR-099)
   ...
   The substrate `:ca-shelter` posting-dimension convention is enforced
   AT THE CONSUMER — postings in TFSA / RRSP / FHSA
...
(defn register!
  "Register the CA investment-income compute-fns with `kontor.tax.statute`.
...
(defn- individual-component
  ...
  (let [scoped-ctx (-> ctx
        ...
        ;; base via its own bracket schedule.
