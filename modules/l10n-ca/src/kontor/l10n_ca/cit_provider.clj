(ns kontor.l10n-ca.cit-provider
  "CA corporate income tax provider — federal T2 Part I + per-province
   ...
   as-data substrate (ADR-101; research note 111; ADR-107). Sibling
   of the DE CIT provider (ADR-104) — same shape (read parameters,
   call `kontor.statute/apply-provisions`, fold components), CA-specific
   ...
   2. For each component (federal `:ca-fed` plus one `:ca-on` /
      ...
      `:component` in ctx, calls `kontor.statute/apply-provisions`
   ...
   Per research note 111 §4 (Finding 2), allocation across provinces is
   **content the provider computes**, not a substrate primitive. The
   ...
   `marginalize` over the appropriate `:posting-dimension`s (note 111
   §3.5 / Finding 3).
   ...
   Out of scope for v1 (note 111 §4 Finding 4-5): RDTOH / Part IV
...
(defn register!
  "Register the CA CIT compute-fns with `kontor.statute`. Called
...
(defn- province-component
  "Build one provincial component map. Base = taxable income ×
   ...
   modelled — note 111 deferred scope)."
