(ns kontor.l10n-ca.cgt-statute
  "CA capital-gains tax — encoded as `kontor.tax.statute` data per ADR-101.
...
(def parameter-values
  "CA CGT parameter values. 2026 LCGE per published rate tables
   (s.110.6(2) ITA + s.117.1 indexation)."
  [;; Inclusion rate — stable 1/2 since 1972 reform. The proposed 2/3
   ...
   {:kontor.parameter-value/parameter      [:kontor.parameter/code "CA.CGT.lcge-cap"]
    ...
    :kontor.parameter-value/citation       "ITA s.110.6(2) — LCGE indexed to $1,275,000 for 2026 disposals (s.117.1 indexation resumed); per published LCGE rate tables"}
