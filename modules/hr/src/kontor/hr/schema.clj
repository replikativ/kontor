  ...
  "kontor-hr companion schema — ADR-075 (Stage R substrate).
   ...
   Entities:
     ...
     :employment              — the legal employment relationship to
                                ...
                                (multi-employer-style multi-employment).
     :department              — recursive per-entity org tree.
     :compensation            — comp envelope for an :employment;
                                lifted off :employment to enable
                                multi-cardinality pay components like
                                Weihnachtsgeld + employer SI + VWL as
                                distinct rows. Effective-dated.
   ...
   plus three minor refinements (:kontor.person/kind for Worker
   subtyping, :kontor.employment/work-time-fraction for FTE,
...
(def ^:private person-attrs
  [{:db/ident       :kontor.person/external-id
    ...
    :db/doc         "Consumer-supplied opaque identifier. Stable
                     across employments and re-hires."}
   ...
   ;; :kontor.person/birth-date is owned by the kernel schema.
   ...
   ;; Worker subtype enum. Open-set: :employee | :contingent |
   ;; :applicant | :retiree | :board-member | :intern |
   ;; <consumer extensions>. Supports contingent-worker / contractor
   ;; payroll without forcing a schema migration when C5+ lands.
   {:db/ident       :kontor.person/kind
    :db/valueType   :db.type/keyword
    :db/cardinality :db.cardinality/one
    :db/doc         "Worker subtype. Open-set keyword;
                     nil = :employee. Generic worker classification
...
;; ============================================================================
;; :kontor.partner/person — kernel↔companion linker
;; ============================================================================
...
(def ^:private partner-person-link-attrs
  [{:db/ident       :kontor.partner/person
    ...
    :db/doc         "Ref to :person, set when :kontor.partner/kind is
                     :employee (or :contingent). The kernel never sees
...
;; ============================================================================
;; :employment — multi-employer-style relationship
;; ============================================================================
  ...
  [{:db/ident       :kontor.employment/code
   ...
   {:db/ident       :kontor.employment/person
    ...
    :db/doc         "Ref to :person. A :person may have N concurrent
                     ...
                     (multi-employer-style; the trans-national pitch —
                     an executive employed by Acme-DE-GmbH AND
                     seconded to Acme-US-LLC — needs this)."}
   ...
   {:db/ident       :kontor.employment/job-title
    ...
    :db/doc         "Free text. :position / :job-profile separation
                     (Job Profile vs Position) deferred."}
...
."}
   ...
   ;; Continuous FTE. Half-FTE part-time vs 80% reduced-hours
   ;; secondment matter for payroll math; :fulltime-flag is too coarse.
   ...
   ;; Extends past the US FLSA exempt/non-exempt binary. Open-set:
   ;; :standard | :secondment | :board-position | :apprentice | :intern
   ;; | :working-student | :civil-servant (DE Beamter) | <consumer ext>.
   {:db/ident       :kontor.employment/work-relationship-kind
    ...
    :db/doc         "Open-set keyword for the employment-relationship
                     ...
                     dedicated value so per-country
                     payroll providers can route correctly."}

   ;; Substrate-level geographic / cadence attrs that CA's T4 + ROE
   ;; emitters required. Promoted to substrate because US
   ;; state-of-employment (W-2 box 15) + DE Bundesland
   ...
   {:db/ident       :kontor.employment/province-of-employment
    ...
    :db/doc         "Geographic locus of the employment for tax /
                     ...
                     The CA T4 builder uses this attr."}
...
;; ============================================================================
;; Modern HR systems model compensation as a SEPARATE entity with
;; multi-cardinality components — base wage + bonus target + employer
;; pension + VWL + housing allowance + RSU vest schedule are
;; simultaneously active, and one scalar attr can't represent them.
;; The bitemporal axis
...
(def ^:private compensation-component-attrs
  [{:db/ident       :kontor.compensation-component/compensation
   ...
   {:db/ident       :kontor.compensation-component/kind
    :db/valueType   :db.type/keyword
    :db/cardinality :db.cardinality/one
    :db/doc         "Open-set per kontor.provider.payroll-provider — common:
...
;; ============================================================================
;; :pay-period — payroll temporal axis
;; ============================================================================
...
;; ============================================================================
;; :consent — per-(subject, scope) legal-basis record (ADR-094,.2)
;; ============================================================================
;;
  ...
  [{:db/ident       :kontor.consent/code
   ...
   {:db/ident       :kontor.consent/scope
    ...
    :db/doc         "Open-set keyword matching the
                     ...
                     (kontor.compliance.audit-doc/canonical-categories). The
...
(def status-transition-seeds
  ...
  (vec
   (concat
    ...
    ;; :kontor.employment/state — multi-employer-style lifecycle.
    ;; :rehired creates a NEW :employment row, but the prior row may
    ...
    ;; :kontor.consent/state — ADR-094. :proposed = DPIA drafted
