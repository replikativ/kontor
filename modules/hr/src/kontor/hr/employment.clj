(ns kontor.hr.employment
  "kontor-hr :employment transactors — the multi-employer-style relationship.
   ...
   The `hire!` orchestrator composes (per ADR-067 kontor.workflow.process):
   ...
   `kontor.workflow.status-machine/record-status-change-tx-data` so the
...
(defn hire-tx-data
  "Pure tx-data builder for `hire!`. Returns a vector of tx-ops
   ...
   Optional keys:
     ...
     :work-relationship-kind — keyword
...
;; terminate
;; ============================================================================
;; Routes through kontor.workflow.status-machine/record-status-change-tx-data so
;; the ADR-038 approval-policy gate (:requires-supporting-doc on
;; :active|:on-leave → :terminated, seeded in schema.clj) actually
;; fires. Note 85: the prior flat-facet write bypassed the
...
(defn terminate-tx-data
  ...
  (when-not supporting-doc (throw (ex-info ":supporting-doc required (termination letter) — schema.clj :requires-supporting-doc approval policy"
...
;; ============================================================================
;; sum-work-time-fraction — over-allocation helper
;; ============================================================================
