(ns kontor.hr.dsar
  "kontor-hr DSAR collector — extends kontor.compliance.dsar/collect with the
   :kontor.partner/person → :person → :employment → :compensation walk.

   The kernel's `kontor.compliance.dsar/collect` is partner-keyed (it walks
...
(defn collect-employee
  "DSAR convenience: walk both the kernel side (partner-driven) and
   ...
   Same `:as-of-tx`, `:include-merged?` opts as kontor.compliance.dsar/collect."
