...
- **`:person`** — global, employment-independent identity (ADR-075).
  ...
  :board-member :intern}` (note 81 §9.7).
- **`:employment`** — the Workday-style relationship (`:person` ↔
  ...
  `:terminated` as audit.
- **`:compensation`** — separated entity (note 81 §9.6). Has N
...
- Research note 79 — Stage R plan; note 81 — HR gold-standards
  validation that drove `:compensation`-as-separate-entity +
  `:person/kind` + `:work-time-fraction` decisions; note 85 —
  Stage R C1 review-after; note 86 — Stage R final review-after;
  note 93 — ADR-094 substrate-posture research
