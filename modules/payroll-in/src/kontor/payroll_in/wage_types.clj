(ns kontor.payroll-in.wage-types
  "IN-specific `:component-kind` extensions for kontor-payroll-in
   ...
   wage-type catalog shape (note 84 §10.2; note 83 §9.2).

   Open-set per ADR-071 P2-71-2 + ADR-075's PayrollFacts contract.
   ...
   ## IN wage-type vocabulary (note 79 §5.3 C9 plan + Schedule III)
   ...
   - **Employer accruals**: `:pf-employer` (12% — split EPS 8.33% +
     ...
     actuarial input — note 79 §5.3 deferral), `:bonus-accrual`
...
(def standard-component-kinds
  ...
  {;; ──────────────────────────────────────────────────────────────
   ...
   ;; band when consumer computes — same divergence as note 86 §2.3
...
(defn payable-tag
  "Return the :payable-tag keyword for an employer-side kind (both
   ...
   liability bucket per note 79 §5.3 C9 plan)."
