(ns kontor.payroll-in.compute
  "IN payroll compute providers — file-ingest CSV adapters for
   common Indian payroll engines:

     - **Keka** (`KekaProvider`).
     - **GreytHR** (`GreytHrProvider`).
     - Other engines (ZenHR / ZingHR / SumoPayroll / Saral PayPack)
       handled via the generic `:column-mapping` shape (engine name
       passed in opts for audit only).

   File-ingest CSV parser, per-customer column-mapping config,
...
(defn csv->facts
  "Group a parsed CSV by employee-external-id and assemble PayrollFacts.
   ...
   The per-fact `:province-of-employment` (when present on a row)
   surfaces in `:jurisdiction-specific-codes` so the posting-builder +
   TDS / PF / ESI builders can route per-state."
...
;; ============================================================================
;; GreytHrProvider — GreytHR GL export
;; ============================================================================

(def greythr-default-column-mapping
  "GreytHR's default GL-export column names (per public help-center
   documentation)."
