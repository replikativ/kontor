...
substrate posture (research note 93 §3.5 + §8 scope tables); this
companion is the minimal v0 implementation:
...
- Activity monitoring (keystrokes, screen, productivity scoring)
  — note 93 §3.3-3.4 calls this out as a separate companion,
...
- [ADR-068](../../doc/decisions.md) — `*-tx-data` pure builders +
  ...
  with-validation`
- Research note 93 — ADR-094 substrate-posture research §3.5 +
  §8 (scope-in vs scope-out tables)
...
- **No activity monitoring.** Keystroke / screen / webcam capture
  is deliberately a separate (unshipped) companion per note 93
  §3.3-3.4. This module is forensically-correct *recorded events*
  only.
