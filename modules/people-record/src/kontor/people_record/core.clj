(ns kontor.people-record.core
  "kontor-people-record — minimal consent + retention + audit-doc +
   ...
   - kontor.compliance.audit-doc: provides the `:audit-doc` backbone +
     canonical categories (ADR-094 §3.1).
   - kontor.compliance.dsar: bundles a person's records via the kernel walker."
...
(defn install!
  "Idempotent install — kernel + kontor-hr attrs must be present (run
   ...
   Registers a `kontor.compliance.dsar` extension collector under `:people-
   record` so the kernel-canonical `kontor.compliance.dsar/collect` walk reaches
   the track-record bundle. Same pattern as `kontor.hr.core/install!`:
   given a partner eid, we resolve the linked
...
(defn check-consent!
  "Throw `:kontor.consent/missing` if no active `:hr-track-record` consent
   ...
   their own `kontor.workflow.process` step list can apply the same consent
...
(defn dsar-bundle
  "Collect every track-record entity touching `person`. Returns a map
   ...
   Consumer DSAR pipelines compose this with kontor.compliance.dsar/collect for
   the kernel + HR-side bundle (per ADR-052)."
  [db person]
