(ns kontor.import-gleif.core
  "kontor-import-gleif — ingest GLEIF Golden Copy LEI Level 1 (entity
   ...
   License posture (ADR-005): GLEIF data is **CC0 1.0
...
;; ============================================================================
;;
;; Full LEI semantic validation (ISO/IEC 7064 MOD 97-10 check digits) is a
...
(defn valid-lei?
  "Returns true iff `s` is 20-character uppercase alphanumeric — the
   ...
   Per ADR-005, semantic validation is GLEIF-side;
...
(defn import-level-1!
  ...
  ([conn rows-or-source opts]
   (let [rows (if (or (instance? java.io.File rows-or-source)
     ...
     ;; would add per-row overhead with nothing to validate.
