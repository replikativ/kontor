(ns kontor.payroll-jp.compute
  "JP payroll compute providers — file-ingest CSV adapters for the
   ...
   - `FreeeProvider` — freee人事労務 CSV export. The CSV column layout
     is per-customer-configurable, so a `:column-mapping` opts map
     carries the load-bearing field names.

   - `MoneyForwardProvider` — Money Forwardクラウド給与 CSV export.
     Similar shape to freee but column headers differ.
