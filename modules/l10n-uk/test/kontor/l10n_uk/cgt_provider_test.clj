(ns kontor.l10n-uk.cgt-provider-test
  "Tests for the UK CGT providers (ADR-102 + ADR-101).
  ...
  (let [conn (core/create-test-db)]
    ...
    (d/transact conn [{:kontor.commodity/symbol "GBP" :kontor.commodity/name "Pound sterling"
                      ...
                      {:kontor.entity/code "HOLDCO" :kontor.entity/name "HoldCo"
                       :kontor.entity/kind :company :kontor.entity/country "GB"
                       :kontor.entity/functional-commodity [:kontor.commodity/symbol "GBP"]}])
    conn))
...
;; ============================================================================
;; §2. Worked example — Jane's BADR sale
;; ============================================================================

(deftest §2-1-jane-badr-sale
    ...
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :individual p2024-25
        ...
        (is (== 195256M (-> cmp :liability :amount))
            "Total CGT matches.1 to the penny")
    ...
    (let [conn (fresh)]
      ...
      (let [facts (run-provider conn :corporation p2024-25)
        ...
        (is (== 149800M (-> cmp :base :amount))
            "matches.2 chargeable-gain figure")
