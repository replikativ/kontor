...
, so they are siblings, not branches of one record:
   ...
   - **`uk-individual-cgt-provider`** — standalone CGT with its own
     ...
     residential aligned post Autumn Budget 2024,1)
...
2 recommendation; multi-tranche holdings emit one
     `:disposal` per tranche.

   ## DisposalProvider

   Both providers depend on the kernel `DisposalProvider` protocol
...
.3 + §5.4, the provider accepts the prior cumulative
   amount via `:tax-unit`:
...
(defn- realized-gain
  "Per-disposal realised gain (positive) or loss (negative), in the
   ...
   ALREADY-INDEXED cost base (consumer responsibility2).
