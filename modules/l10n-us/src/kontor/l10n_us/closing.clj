(ns kontor.l10n-us.closing
  "US fiscal-year close — rolls P&L accounts to retained earnings
   using the kontor default account convention.
   ...
   seed kontor default chart. Callers can pass `:retained-code` to redirect
...
(defn- account-by-code [db code]
  (kacct/resolve-code db code {:context "US year-end closing"}))
...
(defn close-us-fiscal-year!
  "Close a US fiscal-year period using the kontor default
   account convention.
