...
;; The kernel's report engine defaults every line's commodity to :EUR
;; (kontor.reporting.report + financial-statements). On a USD book that
;; produces correct arithmetic tagged with the wrong currency, silently,
;; so every line and section below is stamped explicitly. Drop this once
;; the engine derives the default from the entity's functional commodity.
(defn- in-usd [statement]
  (update statement :statement/sections
          (fn [sections]
            (mapv (fn [s]
                    (-> s
                        (assoc :section/commodity :USD)
                        (update :section/lines
                                (fn [ls] (mapv #(assoc % :line/commodity :USD) ls)))))
                  sections))))
