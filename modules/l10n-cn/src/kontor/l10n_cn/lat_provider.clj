(ns kontor.l10n-cn.lat-provider
  "Land Appreciation Tax (土地增值税, LAT) — `PeriodTaxProvider`
   (ADR-099) over the ADR-102 disposal substrate. 
   §1.6 + §5.3.
   ...
   individual is not a developer). Note 145 §1 fix.
   ...
   Provider depends on `kontor.provider.disposal-provider/DisposalProvider` —
...
(defrecord CnLatProvider [id source authority commodity statute]
  ...
  (period-tax-facts [_ {:keys [entity period] :as ctx}]
    ...
    (let [disposals (ds/disposals-facts source {:entity entity :period period})
          ...
          ;; construction — see namespace docstring +.
          eligible  (->> disposals
                         (filter lat-eligible?)
