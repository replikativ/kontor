(ns kontor.l10n-cn.investment-income-provider-test
  "Tests for the CN IIT + EIT investment-income providers (ADR-099 +
   ...
   - Worked examples from note 158 §2."
...
(deftest statute-parameters-installed
  (testing "IIT/EIT investment-income parameters loaded; current-effective values"
    (let [conn (fresh)
          db   (d/db conn)
          ;; Use kontor.statute/parameter-value-at — the same path the
          ;; provider takes — to validate effective-window resolution.
          at   #inst "2026-01-01"
          pv   (fn [c] (kontor.statute/parameter-value-at db c at))]
...
;; ============================================================================
;; §2. IIT — listed A-share gradation (Caishui [2015] 101) — note 158 §2 ex A
;; ============================================================================

(deftest iit-listed-a-share-le-1m-full-rate
  (testing "≤ 1 month holding → 20 % full (lot L3 from note 158 §2 ex A)"
    (let [conn (fresh)
          facts (run-iit conn
                         [{:event-id     "catl-L3"
...
(deftest iit-listed-a-share-1m-1y-half-rate
  (testing "1m–1y holding → 10 % effective (lot L2 from note 158 §2 ex A)"
    (let [conn (fresh)
          facts (run-iit conn
                         [{:event-id     "catl-L2"
                           :income-class :listed-a-share-dividend
...
(deftest iit-listed-a-share-gt-1y-exempt
  (testing "> 1 year holding → fully exempt (lot L1 from note 158 §2 ex A)"
    (let [conn (fresh)
          facts (run-iit conn
                         [{:event-id     "catl-L1"
                           :income-class :listed-a-share-dividend
...
(deftest iit-listed-a-share-all-three-bands-mixed
  (testing "note 158 §2 ex A: Ms Liu's three lots aggregate to CNY 340"
    (let [conn (fresh)
          facts (run-iit conn
...
;; ============================================================================
;; §7. EIT — §26(2) inter-TRR dividend exemption (note 158 §2 ex B)
;; ============================================================================
